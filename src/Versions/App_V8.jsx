...
async function analyserDossierTest() {
  try {
    const response = await fetch(
      "http://localhost:3001/analyser-dossier",
      {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          chemin:
            "C:\\Test classification musée Capetown"
        }),
      }
    );
    ...
    if (data.success) {
      setDossierImport(data.nomDossier);
      setNombrePhotos(data.nombrePhotos);
    } else {
      alert(data.error);
    }
  } catch (error) {
...
const creerCategoriesMusee = async () => {
  try {
    const response = await fetch(
...
{modeCreationVisite && (
  ...
  >
    ...
    <button
      onClick={() => {
        if (!nomNouvelleVisite.trim()) return;
...
const categorie =
  classificationResult.categorie;

  alert("Catégorie IA = " + categorie);

const cheminClasse =
  `${cheminCible}\\${categorie}`;
...
>
  ...
  {dossierImport && (
    <div style={{ marginTop: "20px" }}>
      ...
      <button
  onClick={() => console.log("CLASSIFICATION")}
>
