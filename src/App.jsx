...
const VERSION = {
  numero: "v30.2",
  descriptif: "correctif Android rangement photos",
  date: "2026-07-09",
  build: 1,
};
...
// PhotoCartel v30.2 — correctif Android rangement photos.
// Objectif : déplacer les photos de Collecte Photo en cours vers la visite métier correspondante.
...
const afficherMessageDiscretArborescence = (message, duree = 3000) => {
  // v28.3.2 : confirmation de succès non bloquante stabilisée à 3 secondes.
  ...
  async function obtenirDossierVisiteAndroid(dossierPhotoCartel, visite) {
    ...
    const dossierVille = await dossierVoyage.getDirectoryHandle(nettoyerNomDossierLocal(visite.ville), { create: true });
  ...
  async function rangerPhotosVisitesAndroid(visitesAtraiter) {
    // v30.2 : rangement Android en deux temps.
...
let photoCartelHandleRacineAndroidSession = null;
...
async function handleAndroidEncoreAutorise(handle) {
  ...
  try {
    ...
    // v30.2 : Chrome Android peut renvoyer "prompt" sur un handle pourtant mémorisé.
    // On demande alors explicitement la réactivation du même handle au lieu de rouvrir
    // systématiquement le sélecteur de dossier, ce qui réduira les demandes répétées.
...
async function handleDcimEncoreAutorise(handle) {
  return handleAndroidEncoreAutorise(handle);
}

async function obtenirDossierRacinePhotoCartelAndroid({ ouvrirSelecteurSiNecessaire = true } = {}) {
  ...
  if (
    photoCartelHandleRacineAndroidSession &&
    (await handleAndroidEncoreAutorise(photoCartelHandleRacineAndroidSession))
  ) {
    return {
      type: "photocartel",
      dossierPhotoCartel: photoCartelHandleRacineAndroidSession,
      source: "session",
    };
  }

  const handleRacineSauvegarde = await lireHandleAndroid(NOM_HANDLE_RACINE_ANDROID);
  if (handleRacineSauvegarde && (await handleAndroidEncoreAutorise(handleRacineSauvegarde))) {
    photoCartelHandleRacineAndroidSession = handleRacineSauvegarde;
    return {
      type: "photocartel",
  ...
  if (photoCartelHandleDcimSession && (await handleDcimEncoreAutorise(photoCartelHandleDcimSession))) {
    const { dossierPhotoCartel } = await creerArborescenceInfrastructurePhotoCartel(
      photoCartelHandleDcimSession
    );
    // v30.2 : quand la racine conservée est DCIM, on reconstruit PhotoCartel à partir de DCIM.
    // On ne persiste pas le sous-handle dérivé PhotoCartel, moins fiable sur Android.
    photoCartelHandleRacineAndroidSession = null;
    return {
      type: "dcim",
      dossierDcim: photoCartelHandleDcimSession,
      dossierPhotoCartel,
      source: "session-dcim",
    };
  }
  ...
  if (handleDcimSauvegarde && (await handleDcimEncoreAutorise(handleDcimSauvegarde))) {
    photoCartelHandleDcimSession = handleDcimSauvegarde;
    const { dossierPhotoCartel } = await creerArborescenceInfrastructurePhotoCartel(
      handleDcimSauvegarde
    );
    // v30.2 : on garde le handle DCIM comme référence stable et on reconstruit PhotoCartel.
    photoCartelHandleRacineAndroidSession = null;
    return {
      type: "dcim",
  ...
  if (dossierChoisi.name === "PhotoCartel") {
    photoCartelHandleRacineAndroidSession = dossierChoisi;
  ...
  if (dossierChoisi.name === "DCIM") {
    photoCartelHandleDcimSession = dossierChoisi;
    ...
    // v30.2 : sélection DCIM = on sauvegarde DCIM uniquement.
...
async function enregistrerAnalysePhoto() {
  if (analysePhotoSauvegardee) {
    setMessageAnalysePhoto("Cette analyse a déjà été enregistrée.");
    return;
  }

  if (!analysePhotoFile || !analysePhotoResultat) {
    setMessageAnalysePhoto("Aucune analyse à enregistrer.");
    return;
  }
  ...
  try {
    ...
    if (estAndroid()) {
      data = await enregistrerAnalysePhotoDansPhotosAnalyseesAndroid({
        fichier: analysePhotoFile,
        analyse: analysePhotoResultat,
      });
    } else {
      const formData = new FormData();
    ...
    setMessageAnalysePhoto(
      "✅ Analyse sauvegardée dans Photos analysées : " +
        (data.nomPhoto || "photo") +
        " + " +
        (data.nomJson || "json")
    );
    ...
    console.error(error);
    setMessageAnalysePhoto("Erreur sauvegarde analyse : " + error.message);
  } finally {
    setAnalysePhotoSauvegardeEnCours(false);
  }
}
...
function ouvrirSelectionActualisationPhotos() {
  // v30.2 : le bouton Ranger ne demande plus de sélectionner des fichiers.
...
async function handlePhotosPrises(event) {
  ...
  async function finDeVisite() {
    ...
    try {
      ...
      // v30.2 : clôture de la fenêtre temporelle de la visite en cours.
      // Règle métier : première photo après Nouvelle visite, dernière photo avant Fin de visite.
      cloturerVisitePourRangement({
        voyageNom: voyage,
        villeNom: villeVisite,
        visiteNom: lieuVisite,
        finMs: maintenant.getTime(),
        nombrePhotos: photosCollectees || 0,
      });

      // Le nouveau dossier tampon est aussi une fenêtre ouverte, pour éviter toute photo orpheline.
  ...
  localStorage.setItem("photoCartelPhotosCollectees", "0");
      ...
      afficherMessageDiscretArborescence(
        estTamponActif
          ? "✅ Dossier tampon clôturé. Nouvelle collecte prête."
          : "✅ Visite clôturée. Nouvelle collecte prête."
      );
    } catch (error) {
      console.error(error);
...
const validerNouvelleVisite = async () => {
  ...
  try {
    ...
    // v30.2 : fermeture éventuelle du dossier tampon actif, puis ouverture de la vraie visite
    ...
    afficherMessageDiscretArborescence("✅ Visite créée : " + nomVisite);
