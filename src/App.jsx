...
const VERSION = {
  numero: "v40.1",
  descriptif: "Modifier l’identité d’une visite",
  date: "2026-07-18",
  build: 1,
};
...
function App() {
  ...
  async function verifierServeurModificationVisite() {
    ...
    if (String(informations.version || "") !== "v40.1") {
      throw new Error(
        `Le serveur actif annonce ${informations.version || "une version inconnue"}, pas v40.1. ` +
  ...
  async function appelerModificationIdentiteVisite(payload) {
    ...
    for (const route of routes) {
      try {
        ...
        const resultat = await lireReponseJsonPhotoCartel(response, "Erreur modification de la visite");
    ...
    throw derniereErreur || new Error("Modification impossible.");
  }
...
const afficherMessageDiscretArborescence = (message, duree = 5000) => {
  ...
  function visiteRapideSelonIdentite(visite = {}) {
    ...
    const nom = String(visite.nom || "").trim();
    return !type || nom.startsWith("Visite rapide_") || nom.startsWith("A_EN_COURS_");
  }
  ...
  function ouvrirModificationIdentiteVisite(visite = null, source = "resume") {
    ...
    setTypeVisiteModifie(estRapide ? "Autre" : String(cible.type || ""));
  ...
  async function copierDossierAndroidRecursive(source, destination, ancienNom, nouveauNom) {
    for await (const [nom, handle] of source.entries()) {
      const nomCible = remplacerNomDansNomFichier(nom, ancienNom, nouveauNom);
      if (handle.kind === "directory") {
        const sousDossier = await destination.getDirectoryHandle(nomCible, { create: true });
        await copierDossierAndroidRecursive(handle, sousDossier, ancienNom, nouveauNom);
      } else {
        const fichier = await handle.getFile();
        const cible = await destination.getFileHandle(nomCible, { create: true });
  ...
  async function modifierIdentiteVisiteAndroid(payload) {
    const resultatRacine = await obtenirRacinePhotoCartelAndroid({ demanderSiNecessaire: true });
    ...
    const voyageHandle = await voyages.getDirectoryHandle(nettoyerNomDossierLocal(payload.voyage), { create: true });
    const ancienneVilleStockage = payload.ancienneVisiteRapide ? "Visites rapides" : payload.ancienneVille;
    const nouvelleVilleStockage = payload.nouvelleVisiteRapide ? "Visites rapides" : payload.nouvelleVille;
    const ancienParent = await voyageHandle.getDirectoryHandle(nettoyerNomDossierLocal(ancienneVilleStockage), { create: false });
    const ancienDossier = await ancienParent.getDirectoryHandle(nettoyerNomDossierLocal(payload.ancienNom), { create: false });
    const nouveauParent = await voyageHandle.getDirectoryHandle(nettoyerNomDossierLocal(nouvelleVilleStockage), { create: true });
    const nouveauDossier = await nouveauParent.getDirectoryHandle(nettoyerNomDossierLocal(payload.nouveauNom), { create: true });

    await copierDossierAndroidRecursive(ancienDossier, nouveauDossier, payload.ancienNom, payload.nouveauNom);
    for (const categorie of categoriesPourTypeVisiteLocale(payload.nouveauType)) {
      await nouveauDossier.getDirectoryHandle(categorie, { create: true });
    }
    await ancienParent.removeEntry(nettoyerNomDossierLocal(payload.ancienNom), { recursive: true });

    return { success: true, chemin: `${payload.voyage}\\${nouvelleVilleStockage}\\${payload.nouveauNom}` };
  }
  ...
  async function enregistrerModificationIdentiteVisite() {
    ...
    const nouveauType = typeAffiche === "Autre" ? "" : typeAffiche;
    const nouveauRapide = !nouveauType && ville === "Ville non renseignée";
    const payload = {
    ...
    try {
      ...
      const historiqueMaj = historique.map((v) => {
        const correspond = ancienne.id ? v.id === ancienne.id : (!v.finMs && v.nom === ancienne.nom && v.voyage === ancienne.voyage);
        if (!correspond) return v;
        return { ...v, nom, ville, type: nouveauType, chemin: resultat.chemin || v.chemin };
      });
      ecrireHistoriqueVisitesRangement(historiqueMaj);
      const visiteMaj = historiqueMaj.find((v) => ancienne.id ? v.id === ancienne.id : (!v.finMs && v.nom === nom)) || { ...ancienne, nom, ville, type: nouveauType };
      ...
      if (ancienne.estActive) {
        ...
        setVisiteActive((courante) => courante ? { ...courante, nom, ville, type: nouveauType, chemin: resultat.chemin || courante.chemin } : courante);
      }
  ...
  function visitesRangeesRecentes() {
    return lireHistoriqueVisitesRangement()
      .filter((visite) => visite && visite.rangee && visite.finMs)
      .sort((a, b) => Number(b.dateRangementMs || b.finMs || 0) - Number(a.dateRangementMs || a.finMs || 0))
      .slice(0, 12);
  }
...
async function obtenirDossierRacinePhotoCartelAndroid({
  ...
  // v38.4 : un nouvel identifiant de sélecteur neutralise l’ancien emplacement
  // mémorisé par Chrome (par exemple « Collecte Photo en cours »).
  // Quand un handle racine connu existe encore, il sert de point de départ :
  // le sélecteur s’ouvre alors directement sur PhotoCartel ou DCIM.
...
const validerNouvelleVisite = async () => {
  ...
  const baseStyles = {
    ...
    boutonModifierIdentite: {
      ...
      background: "#fff8e8",
      color: "#9b6418",
      boxShadow: "0 2px 6px rgba(91,67,38,0.12)",
      cursor: "pointer",
      fontFamily: themePhotoCartel.font,
      fontSize: "15px",
      fontWeight: "900",
    ...
    resumeVisiteLigne: {
      display: "grid",
      gridTemplateColumns: "25px minmax(0, 1fr) minmax(82px, auto)",
      alignItems: "center",
      gap: "7px",
      minHeight: "39px",
      padding: "6px 2px",
    ...
    resumeVisiteDatesCadre: {
      ...
      margin: "8px 6px",
      padding: "5px 9px",
      border: "1px solid rgba(199,166,110,0.32)",
      borderRadius: "13px",
    ...
    resumeVisiteDateLigne: {
      ...
      minHeight: "27px",
      padding: "1px 0",
      borderBottom: "1px solid rgba(91,67,38,0.08)",
    },
  ...
  function LigneResumeVisite({ icone, label, valeur, separation = false }) {
    return (
      <div
        style={{
      ...
      >
        <div style={styles.resumeVisiteIcone}>{icone}</div>
        <div style={styles.resumeVisiteLabel}>{label}</div>
        <div style={styles.resumeVisiteValeur}>{afficherValeurEtat(valeur)}</div>
      </div>
    );
  }
      ...
      <section style={styles.resumeVisiteCarte} aria-label="Résumé de visite">
        <div style={styles.resumeVisiteTitreLigne}>
          ...
          <button
            ...
            style={styles.boutonModifierIdentite}
        ...
        <LigneResumeVisite icone="🏛️" label="Nom de la visite" valeur={visite.nom} />
        ...
        />
        <LigneResumeVisite icone="🏛️" label="Type de visite" valeur={libelleTypeVisiteResume(visite)} />
        ...
        <LigneResumeVisite
          ...
          valeur={Number(visite.photosRangees ?? visite.nombrePhotosDeclare ?? 0)}
        />

        <div style={styles.resumeVisiteStatutZone}>
          <LigneResumeVisite icone="✅" label="Statut" valeur="Rangée" />
        </div>

        <div style={styles.resumeVisiteBoutonsSecondaires}>
          <button type="button" disabled style={styles.resumeVisiteBoutonSecondaire}>
            📂 Ouvrir le dossier
          </button>
          ...
          <div style={styles.accueilInformations}>
            <div style={styles.accueilVoyageTitreLigne}>
              <div style={styles.accueilVoyageTexte}>
                <div style={styles.accueilTitreAvecEdition}>
        ...
        <div style={{ ...styles.modalOverlay, zIndex: 10030 }}>
          <div style={styles.modal} role="dialog" aria-modal="true" aria-labelledby="titre-modifier-identite-visite">
            ...
            <label><strong>Nom de la visite</strong>
              <input type="text" value={nomVisiteModifie} onChange={(e) => setNomVisiteModifie(e.target.value)} style={styles.input} />
            </label>
            <label><strong>Ville</strong>
              <input type="text" value={villeVisiteModifiee} onChange={(e) => setVilleVisiteModifiee(e.target.value)} style={styles.input} />
            </label>
            <div style={{ position: "relative", marginBottom: "14px" }}>
              <strong style={{ display: "block", marginBottom: "6px" }}>Type de visite</strong>
              <button type="button" onClick={() => setListeTypesModificationOuverte((v) => !v)} style={{ ...styles.input, width: "100%", margin: 0, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span>{typeVisiteModifie}</span><span>{listeTypesModificationOuverte ? "⌃" : "⌄"}</span>
              </button>
              {listeTypesModificationOuverte && (
                <div style={{ position: "fixed", left: "50%", top: "50%", transform: "translate(-50%, -50%)", zIndex: 10040, width: "min(420px, calc(100vw - 34px))", maxHeight: "calc(100dvh - 190px)", overflowY: "auto", padding: "10px", borderRadius: "16px", background: "#fffdf8", boxShadow: "0 18px 42px rgba(34,26,16,.24)" }}>
                  <div style={{ textAlign: "center", fontWeight: 800, padding: "4px 0 8px" }}>Sélectionner un type de visite</div>
                  {["Musée","Église","Transport","Site naturel","Ville / Village","Jardin / Parc","Architecture","Château","Restaurant / Repas","Autre"].map((type) => (
                    <button key={type} type="button" onClick={() => { setTypeVisiteModifie(type); setListeTypesModificationOuverte(false); }} style={{ width: "100%", minHeight: "38px", border: "none", borderTop: "1px solid rgba(181,138,58,.13)", background: typeVisiteModifie === type ? "rgba(181,138,58,.11)" : "transparent", textAlign: "left", padding: "7px 8px", fontFamily: "inherit", fontSize: "14px" }}>
                      {typeVisiteModifie === type ? "● " : "○ "}{type}
                    </button>
                  ))}
                </div>
              )}
        ...
        {resultatClassification && !dashboardRenommage ? (
          ...
          <>
            ...
            <section style={styles.dernieresVisitesAccueil}>
              ...
              <button
                type="button"
                onClick={() => setListeDernieresVisitesOuverte((ouverte) => !ouverte)}
              ...
              {listeDernieresVisitesOuverte && (
                <div id="liste-dernieres-visites-rangees" style={styles.dernieresVisitesListe}>
                  ...
                  {visitesRangeesRecentes().length > 0 ? (
                    ...
                    <div style={styles.dernieresVisitesVide}>
                      Aucune visite rangée disponible pour le moment.
                    </div>
                  )}
                </div>
              )}
            ...
            {visiteResumeSelectionnee && <BlocResumeVisite visite={visiteResumeSelectionnee} />}
