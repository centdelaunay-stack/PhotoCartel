...
const VERSION = {
  numero: "v28.2.5",
  descriptif: "dossiers métier - création réelle Android de la visite",
  date: "2026-07-07",
  build: 5,
};
...
// PhotoCartel v28.2.5 — dossiers métier : création réelle Android de la visite.
...
async function creerDossiersNouvelleVisiteAndroid({ voyageNom, villeNom, visiteNom, typeVisiteNom }) {
  // v28.2.5 : création réelle sur Android dans DCIM/PhotoCartel/Voyages/<Voyage>/<Ville>/<Visite>.
  // Point clé : on ne passe pas par Render pour créer les dossiers du téléphone.
  // L'utilisateur sélectionne exactement DCIM, puis PhotoCartel ouvre/crée PhotoCartel
  // et ajoute les dossiers métier manquants de manière idempotente.
  if (!estAndroid()) {
    return {
      success: false,
      ignore: true,
  ...
  setMessageArborescenceAndroid(
    "Sélecteur Android ouvert : sélectionne exactement DCIM. PhotoCartel créera ensuite la visite dans DCIM/PhotoCartel/Voyages."
  );
  ...
  const dossierDcim = await window.showDirectoryPicker({
    id: "photocartel-dcim-v28-2-5-visite",
    mode: "readwrite",
    startIn: "pictures",
  });
...
const validerNouvelleVisite = async () => {
  // v28.2.5 : création de la visite métier.
