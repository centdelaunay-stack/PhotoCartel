...
const VERSION = {
  numero: "v28.2.7",
  descriptif: "dossiers métier - permissions Android stabilisées",
  date: "2026-07-07",
  build: 7,
};
...
// PhotoCartel v28.2.7 — dossiers métier : création réelle Android de la visite.
...
let photoCartelHandleDcimSession = null;

async function obtenirDossierDcimAndroid() {
  // v28.2.7 : point unique d'autorisation Android.
  // On ne demande plus l'autorisation à la création du voyage.
  // Pendant une même session, on réutilise le handle DCIM déjà autorisé.
  if (photoCartelHandleDcimSession) {
    try {
      if (photoCartelHandleDcimSession.queryPermission) {
        const permission = await photoCartelHandleDcimSession.queryPermission({
          mode: "readwrite",
        });

        if (permission === "granted") {
          return photoCartelHandleDcimSession;
        }
      }
    } catch (error) {
      console.warn("Handle DCIM de session non réutilisable :", error);
    }

    photoCartelHandleDcimSession = null;
  }
  ...
  photoCartelHandleDcimSession = dossierDcim;
  return dossierDcim;
}

async function creerDossiersNouvelleVisiteAndroid({ voyageNom, villeNom, visiteNom, typeVisiteNom }) {
  // v28.2.7 : création réelle sur Android dans DCIM/PhotoCartel/Voyages/<Voyage>/<Ville>/<Visite>.
  // L'autorisation Android est demandée uniquement ici, jamais à la création du voyage.
  // Si un handle DCIM est déjà autorisé pendant la session, il est réutilisé.
  if (!estAndroid()) {
    return {
      success: false,
      ignore: true,
  ...
  setMessageArborescenceAndroid(
    "Création Android : sélectionne DCIM si Android le demande. PhotoCartel créera ensuite la visite dans DCIM/PhotoCartel/Voyages."
  );

  const dossierDcim = await obtenirDossierDcimAndroid();
...
const validerNouveauVoyage = async () => {
  ...
  try {
    ...
    setMessageArborescenceAndroid("Voyage actif : " + nomVoyage);
...
const validerNouvelleVisite = async () => {
  // v28.2.7 : création de la visite métier.
  ...
  try {
    ...
    setMessageArborescenceAndroid("Dossier visite créé : " + cheminVisiteFinal);
