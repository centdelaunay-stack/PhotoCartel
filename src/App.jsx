...
const VERSION = {
  ...
  descriptif: "Recherche : toutes les photos, un seul champ, résultats ordonnés, sans attente",
  date: "2026-09-11",
  build: 5,
};
...
// v78 — miniature d'une photo trouvée. Composant de niveau module : son identité
// ne change pas d'un rendu à l'autre, donc une miniature déjà affichée n'est
// jamais rechargée. L'image vient de la fonction « obtenir » (serveur sur PC,
// fabriquée puis gardée dans le navigateur sur le téléphone).
function MiniatureRecherche({ fichier, obtenir, taille = 64, arrondi = 12 }) {
  const [url, setUrl] = useState("");
  useEffect(() => {
    let actif = true;
    setUrl("");
    if (fichier) {
      obtenir(fichier).then((valeur) => {
...
const [rechercheVue, setRechercheVue] = useState("liste");
const [rechercheLigneOuverte, setRechercheLigneOuverte] = useState(null);
const [rechercheNombreLignes, setRechercheNombreLignes] = useState(40);
const [rechercheNombreGrille, setRechercheNombreGrille] = useState(90);
...
function ouvrirEcranRecherche() {
  ...
  setMessageMenuAccueil("");
  setCibleMessageMenuAccueil("");
  setRechercheVue("liste");
  setRechercheLigneOuverte(null);
...
// Image d'une photo trouvée. PC : le serveur la sert depuis la racine (avec
// une taille, c'est une miniature). Téléphone : la miniature est fabriquée au
// premier affichage puis gardée dans le navigateur ; l'image entière est lue
// à la demande.
async function lireFichierPhotoRechercheAndroid(fichierRelatif) {
  const racine = await obtenirRacineRechercheAndroid();
  if (!racine) throw new Error("Le dossier PhotoCartel n’est pas accessible.");
  const segments = String(fichierRelatif || "").split("/").filter(Boolean);
  const nom = segments.pop();
  let dossier = racine;
  for (let i = 0; i < segments.length; i += 1) {
    dossier = await dossier.getDirectoryHandle(segments[i]);
  }
  return (await dossier.getFileHandle(nom)).getFile();
}

async function fabriquerMiniatureRecherche(fichier) {
  const image = await createImageBitmap(fichier);
  const echelle = Math.min(1, TAILLE_MINIATURE_RECHERCHE / Math.max(image.width, image.height));
  ...
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => (blob ? resolve(blob) : reject(new Error("miniature"))), "image/jpeg", 0.78);
  });
}
...
async function obtenirMiniatureRecherche(fichierRelatif) {
  const cle = String(fichierRelatif || "");
  if (!cle) return "";
  const memoire = miniaturesRechercheRef.current.get(cle);
  if (memoire !== undefined) return memoire;
  if (!estAndroid()) {
    const url = urlPhotoRechercheServeur(cle, TAILLE_MINIATURE_RECHERCHE);
    miniaturesRechercheRef.current.set(cle, url);
    return url;
  }
  try {
    let blob = await lireDansBaseRecherche(NOM_STORE_MINIATURES_RECHERCHE, cle);
    if (!blob) {
      blob = await fabriquerMiniatureRecherche(await lireFichierPhotoRechercheAndroid(cle));
      await ecrireDansBaseRecherche(NOM_STORE_MINIATURES_RECHERCHE, cle, blob);
    }
    const url = URL.createObjectURL(blob);
    miniaturesRechercheRef.current.set(cle, url);
    return url;
  } catch (error) {
    miniaturesRechercheRef.current.set(cle, "");
    return "";
  }
}

async function ouvrirPhotoRechercheEnGrand(fichierRelatif) {
  try {
    if (!estAndroid()) {
      setPhotoPleinEcranUrl(urlPhotoRechercheServeur(fichierRelatif, 0));
      return;
    }
    const fichier = await lireFichierPhotoRechercheAndroid(fichierRelatif);
    setPhotoPleinEcranUrl(URL.createObjectURL(fichier));
  } catch (error) {
    setRechercheMessageMot("Cette photo n’est plus dans son dossier.");
  }
}

// Résultat de la recherche courante, recalculé seulement quand la saisie, les
// pastilles ou le moteur changent (pas à chaque rendu de l'application).
  ...
  return (
    ...
    >
      ...
      {modeRechercheResultats && (() => {
        // v78 — écran de recherche. Styles littéraux, tirés de la palette du
        // thème : les deux jeux de styles de l'application sont construits clé
        // par clé, une clé ajoutée à l'un seulement serait indéfinie sur l'autre.
        ...
        // Grille d'un sujet ou d'un dossier ouvert depuis la liste.
        if (rechercheVue === "grille" && rechercheLigneOuverte && moteur) {
          const ligne = rechercheLigneOuverte;
          const dossier = ligne.type === "dossier" && ligne.dossier >= 0 ? moteur.dossiers[ligne.dossier] : null;
          const motsDossier = dossier ? motsAjoutesRechercheRef.current[dossier.chemin] || [] : [];
          const photosGrille = ligne.photos.slice(0, rechercheNombreGrille);
          return (
            <div style={{ ...styles.analyseEcran, paddingBottom: "154px" }}>
              <main style={{ ...styles.analyseTelephone, paddingBottom: "148px", fontFamily: t.font }}>
                <button
                  type="button"
                  style={{ ...styleBoutonSecondaire, marginBottom: "12px" }}
                  onClick={() => {
                    setRechercheVue("liste");
                    setRechercheMessageMot("");
                  }}
                ...
                <h1 style={{ ...styles.titreFicheResultat, marginTop: 0 }}>{ligne.titre}</h1>
                <div style={{ color: t.texteDoux, fontSize: "14px", marginBottom: "12px" }}>
                  {ligne.photos.length} photo{ligne.photos.length > 1 ? "s" : ""}
                  {ligne.dossier >= 0 && ligne.type !== "dossier" ? ` · ${contexteDossierRecherche(ligne.dossier)}` : ""}
                </div>

                {dossier && (
                  <div style={styleCarte}>
                    <div style={styleLibelle}>Mots ajoutés à ce dossier</div>
                    <div style={{ ...styleRangee, flexWrap: "wrap" }}>
                      {motsDossier.length === 0 && (
                      ...
                      {motsDossier.map((mot) => (
                        <button
                          ...
                          onClick={() => retirerMotRecherche(dossier.chemin, mot)}
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      <input
                        ...
                        onKeyDown={(event) => {
                          if (event.key === "Enter") ajouterMotRecherche(dossier.chemin, rechercheSaisieMot);
                        }}
                      ...
                      <button
                        ...
                        onClick={() => ajouterMotRecherche(dossier.chemin, rechercheSaisieMot)}
                      >
                        Ajouter
                      </button>
                    </div>
                ...
                {rechercheMessageMot && (
                  <div style={{ color: "#9b2c2c", fontSize: "14px", marginBottom: "10px" }}>{rechercheMessageMot}</div>
                )}

                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "6px" }}>
                  {photosGrille.map((position) => {
                    const photo = moteur.photos[position];
                    return (
                      <button
                        key={position}
                        type="button"
                        onClick={() => ouvrirPhotoRechercheEnGrand(photo.fichier)}
                        title={photo.nom}
                        style={{ padding: 0, border: "none", background: "none", cursor: "pointer", aspectRatio: "1 / 1" }}
                      >
                        <MiniatureRecherche fichier={photo.fichier} obtenir={obtenirMiniatureRecherche} taille="100%" arrondi={10} />
                      </button>
                    );
                  })}
                </div>
                {ligne.photos.length > photosGrille.length && (
                  <button
                    type="button"
                    style={{ ...styleBoutonSecondaire, width: "100%", marginTop: "12px" }}
                    onClick={() => setRechercheNombreGrille((valeur) => valeur + 90)}
                  >
          ...
          <div style={{ ...styles.analyseEcran, paddingBottom: "154px" }}>
            <main style={{ ...styles.analyseTelephone, paddingBottom: "148px", fontFamily: t.font }}>
              ...
              <div style={{ color: t.texteDoux, fontSize: "14px", margin: "8px 0 12px" }}>
                {!moteur && rechercheEtat === "indisponible"
                ...
                const detail =
                  ligne.type === "photo"
                    ? premiere && premiere.auteur
                      ? premiere.auteur
                      : premiere && premiere.titre
                        ? premiere.nom
                        : ""
                    : ligne.type === "sujet"
                      ? `${ligne.photos.length} prises`
                      : `${ligne.photos.length} photo${ligne.photos.length > 1 ? "s" : ""}${
                          ligne.photosAPart ? ` · et ${ligne.photosAPart} autre${ligne.photosAPart > 1 ? "s" : ""} plus haut` : ""
                        }`;
                return (
                  <button
                    key={ligne.cle}
                    ...
                    onClick={() => {
                      if (ligne.type === "photo") {
                        ouvrirPhotoRechercheEnGrand(premiere.fichier);
                        return;
                      }
                      setRechercheLigneOuverte(ligne);
                      setRechercheNombreGrille(90);
                      setRechercheSaisieMot("");
                      setRechercheMessageMot("");
                      setRechercheVue("grille");
                    }}
                  >
                    <MiniatureRecherche fichier={premiere ? premiere.fichier : ""} obtenir={obtenirMiniatureRecherche} />
                    <span style={{ minWidth: 0, flex: 1 }}>
                      <span style={{ display: "block", fontWeight: 700, color: t.encre, fontSize: "15px", overflowWrap: "anywhere" }}>
                        {ligne.type === "dossier" ? "📁 " : ""}
                        {ligne.titre}
                      </span>
                      {detail && (
                        <span style={{ display: "block", fontSize: "13px", color: t.texte, marginTop: "2px", overflowWrap: "anywhere" }}>
                          {detail}
                        </span>
                      )}
                      {contexte && (
                        <span style={{ display: "block", fontSize: "12px", color: t.texteDoux, marginTop: "2px", overflowWrap: "anywhere" }}>
                          {contexte}
                        </span>
                      )}
                      {ligne.origine && (
                        <span style={{ display: "block", fontSize: "11px", color: t.orFonce, marginTop: "3px" }}>
                          Trouvé par : {ligne.origine}
                        </span>
                      )}
                    </span>
                  </button>
              ...
              {rechercheMessageMot && (
                <div style={{ color: "#9b2c2c", fontSize: "14px", marginTop: "10px" }}>{rechercheMessageMot}</div>
              )}
            </main>
          </div>
        );
