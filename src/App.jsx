...
const VERSION = {
  numero: "v34.2",
  descriptif: "stabilisation des actions de la fiche résultat",
  date: "2026-07-12",
  build: 3,
};
...
// PhotoCartel v34.2 — stabilisation du cycle manuel de l’analyse IA et des actions de la fiche résultat.
...
async function handleAndroidEncoreAutorise(handle) {
  ...
  try {
    ...
    // v30.x : Chrome Android peut renvoyer "prompt" sur un handle pourtant mémorisé.
...
function handleAnalyserUnePhoto() {
  // v32 DEV : commande globale, disponible depuis tous les écrans.
  // Comme « Accueil », elle interrompt le contexte courant ; contrairement à « Accueil »,
  // elle ouvre immédiatement l'étape « Sélectionner une action ».
  fermerAnalysePhoto();
...
async function modifierAnalysePhotoAndroid({ fichier, analyse, timestampInitial, ancienNomPhoto, ancienNomJson }) {
  const resultatRacineAndroid = await obtenirDossierRacinePhotoCartelAndroid();
  const dossierPhotoCartel = resultatRacineAndroid.dossierPhotoCartel;
  const dossier = await dossierPhotoCartel.getDirectoryHandle("Photos analysées", { create: true });
    ...
    const boutons = [
      {
        ...
        texte: "Accueil",
        action: async () => {
          // Sur la fiche d’analyse, Accueil est le même kill switch que
          // « Fermer sans enregistrer » : arrêt, suppression et retour accueil.
          if (modeAnalysePhoto || analysePhotoSessionActiveRef.current) {
            await fermerSansEnregistrerAnalysePhoto();
            return;
          }
  ...
  function BarreActionsFicheResultat() {
    const actions = [
      ...
      },
      {
        ...
        action: fermerSansEnregistrerAnalysePhoto,
        disabled: false,
      },
    ];
      ...
      {analysePhotoEnCours && (
        <div style={styles.modalFond}>
          <div style={styles.modalCarte}>
            ...
            <button type="button" onClick={interrompreAnalysePhotoIA} style={styles.modalBoutonAnnuler}>
