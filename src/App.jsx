// PhotoCartel v42.0.15 — sécurisation PWA : modification d’identité suspendue et audit non destructif de DCIM/PhotoCartel.
...
const VERSION = {
  ...
  descriptif: "Restauration modification identité visite PWA",
  date: "2026-07-21",
  build: 12,
};
...
const afficherMessageDiscretArborescence = (message, duree = 5000) => {
  ...
  function ouvrirModificationIdentiteVisite(visite = null, source = "resume") {
    // v42.0.15 : suspension temporaire et explicite sur PWA Android.
    // Le moteur précédent pouvait laisser coexister source et destination.
    // La fonction reste disponible en local PC, où le serveur transactionnel est utilisé.
    if (estAndroid()) {
      alert(
        "Modification d’identité temporairement désactivée sur la PWA.\n\n" +
          "Cette protection évite de créer de nouveaux doublons pendant l’audit et la remise au propre des dossiers."
      );
      return;
    }
  ...
  async function copierDossierAndroidRecursive(source, destination, ancienNom, nouveauNom) {
    // v42.0.14 : toutes les créations passent par les helpers typés.
    // Chrome Android peut renvoyer TypeMismatchError sous plusieurs formes lorsque
    // le nom demandé existe déjà avec un autre type. On détecte alors l'entrée
    // réellement présente et on choisit un nom voisin sans écraser de donnée.
    const nomsReserves = new Set();

    for await (const [nom, handle] of source.entries()) {
      const nomSouhaite = remplacerNomDansNomFichier(
        nom,
        ancienNom,
        nouveauNom
      );
      const typeSouhaite = handle.kind === "directory" ? "directory" : "file";
  ...
  async function modifierIdentiteVisiteAndroid(payload) {
    ...
    if (memeParent && memeNom) {
      for (const categorie of categoriesPourTypeVisiteLocale(payload.nouveauType)) {
        await obtenirDossierAndroid(ancienDossier, categorie, { create: true });
      }
      return {
        success: true,
    ...
    try {
      await obtenirDossierAndroid(nouveauParent, nouveauNomNettoye, {
        create: false,
      });
      throw new Error("Une visite porte déjà ce nom dans cette ville.");
    } catch (error) {
      if (error?.message === "Une visite porte déjà ce nom dans cette ville.") {
        throw error;
      }
    ...
    let copieDestinationComplete = false;
    try {
      ...
      try {
        ...
        throw new Error(
          "Copie de la visite impossible : " +
            (error?.message || String(error))
        );
      }

      try {
        for (const categorie of categoriesPourTypeVisiteLocale(
          payload.nouveauType
        )) {
          await creerSousDossierAndroidFiable(nouveauDossier, categorie);
        }
      } catch (error) {
        throw new Error(
          "Création des catégories impossible : " +
            (error?.message || String(error))
        );
      }

      copieDestinationComplete = true;

      try {
        await supprimerEntreeAndroidRecursiveFiable(ancienParent, ancienNomNettoye);
      } catch (error) {
        throw new Error(
          "La nouvelle visite a été copiée et conservée, mais l’ancienne n’a pas pu être supprimée : " +
            (error?.message || String(error))
        );
      }
    } catch (error) {
      // Tant que la copie n'est pas complète, la destination inachevée est retirée.
      // Une fois la copie complète, elle est conservée même si la suppression de
      // l'ancienne visite échoue : aucune donnée nouvellement copiée n'est perdue.
      if (destinationCreeeParCetteOperation && !copieDestinationComplete) {
        try {
          await supprimerEntreeAndroidRecursiveFiable(nouveauParent, nouveauNomNettoye);
          ...
          <div style={styles.modal}>
            ...
            <section style={styles.parametresSection}>
              <div style={styles.sectionTitre}>
                Audit des dossiers PhotoCartel — v42.0.15
              </div>
              <p style={styles.parametresTexte}>
                Analyse non destructive de DCIM/PhotoCartel/Voyages : visites,
                dossiers vides, homonymes, candidats doublons, entrées
                temporaires et erreurs de lecture.
              </p>
              <p style={styles.parametresTexte}>
                <strong>Aucune suppression ni modification automatique.</strong>
              </p>

              <button
                type="button"
                onClick={auditerStockagePhotoCartelAndroid}
                disabled={auditStockageAndroidEnCours || !estAndroid()}
                style={{
                  ...styles.bouton,
                  opacity:
                    auditStockageAndroidEnCours || !estAndroid() ? 0.45 : 1,
                  cursor:
                    auditStockageAndroidEnCours || !estAndroid()
                      ? "not-allowed"
                      : "pointer",
                }}
              >
                {auditStockageAndroidEnCours
                  ? "Audit en cours…"
                  : "🔎 Auditer DCIM / PhotoCartel"}
              </button>

              <button
                type="button"
                onClick={exporterRapportAuditStockageAndroid}
                disabled={!rapportAuditStockageAndroid || auditStockageAndroidEnCours}
                style={{
                  ...styles.boutonTraitement,
                  marginTop: 9,
                  opacity:
                    !rapportAuditStockageAndroid || auditStockageAndroidEnCours
                      ? 0.45
                      : 1,
                  cursor:
                    !rapportAuditStockageAndroid || auditStockageAndroidEnCours
