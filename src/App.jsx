...
// PhotoCartel v24-test-stockage-android — test dossier tampon DCIM/EN_COURS_PhotoCartel. Moteurs conservés.
// Moteurs, endpoints et règles métier conservés.

function App() {
  console.log("APP PRINCIPALE - PhotoCartel v24-test-stockage-android cloud-ready");
...
async function testerStockageAndroid() {
  setMessageTestStockageAndroid(
    "Sélecteur Android ouvert : sélectionne exactement DCIM, pas un sous-dossier."
  );
  ...
  try {
    // IMPORTANT v24 : showDirectoryPicker doit être appelé directement depuis le clic utilisateur.
    ...
    if (dossierDcim.name !== "DCIM") {
      throw new Error(
        "Mauvais dossier sélectionné. Dossier reçu : " +
          dossierDcim.name +
          ". Il faut sélectionner exactement DCIM, pas PhotoCartel, pas EN_COURS_PhotoCartel, pas un dossier de visite."
      );
    }
    ...
    etape = "création ou ouverture de DCIM/EN_COURS_PhotoCartel";
    const dossierTampon = await dossierDcim.getDirectoryHandle(
    ...
    const contenuTest =
      ...
      "Chemin cible demandé : DCIM / EN_COURS_PhotoCartel\n" +
    ...
    setMessageTestStockageAndroid(
      "Test réussi : DCIM/EN_COURS_PhotoCartel est créé ou accessible. " +
        "Fichier écrit et relu : " +
        nomFichierTest
    );
  } catch (error) {
  ...
  function BarreSuperieurePhotoCartel() {
    return (
      <header style={styles.barreSuperieure} aria-label="En-tête PhotoCartel">
        ...
        <span style={styles.barreSuperieureVersion}>v24-test-stockage</span>
