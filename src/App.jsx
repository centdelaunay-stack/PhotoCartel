// PhotoCartel v42.0.11 — correction robuste de la copie Android lors de la modification d’identité d’une visite.
...
const VERSION = {
  ...
  descriptif: "Correction modification identité visite Android/PWA",
  date: "2026-07-21",
  build: 10,
};
...
const [cheminTamponActif, setCheminTamponActif] = useState(
  ...
  const [visitesPhysiques, setVisitesPhysiques] = useState([]);
...
const afficherMessageDiscretArborescence = (message, duree = 5000) => {
  ...
  function marquerVisitesRangees(resultatsVisites = []) {
    ...
    const prochainHistorique = historique.map((visite) => {
      ...
      const rangementValide = resultat.rangee === true;
      return {
        ...visite,
  ...
  function nomAlternatifAndroid(nom, index) {
    const valeur = String(nom || "");
    const positionPoint = valeur.lastIndexOf(".");
    if (positionPoint > 0) {
      return `${valeur.slice(0, positionPoint)}_${index}${valeur.slice(positionPoint)}`;
    }
    return `${valeur}_${index}`;
  }

  async function choisirNomCibleAndroid(destination, nomSouhaite, typeSouhaite, nomsReserves) {
    let candidat = nomSouhaite;
    let index = 2;

    while (true) {
      if (nomsReserves.has(candidat)) {
        candidat = nomAlternatifAndroid(nomSouhaite, index++);
        continue;
      }

      const entreeExistante = await lireEntreeAndroid(destination, candidat);
      if (!entreeExistante || entreeExistante.kind === typeSouhaite) {
        nomsReserves.add(candidat);
        return candidat;
      }

      // La destination est créée spécialement pour cette opération.
      // En cas de collision fichier/dossier, on conserve toutes les données
      // en choisissant un nom voisin au lieu de supprimer une entrée.
      candidat = nomAlternatifAndroid(nomSouhaite, index++);
    }
  }
  ...
  async function creerSousDossierAndroidFiable(parent, nomSouhaite) {
    ...
    try {
      ...
      if (estErreurTypeEntreeAndroid(error)) {
        const apresErreur = await lireEntreeAndroid(parent, nomSouhaite);
        if (apresErreur?.kind === "directory") return apresErreur;
        throw new Error(
          `Impossible de créer le dossier « ${nomSouhaite} » : une entrée d’un autre type utilise ce nom.`
        );
      }
      throw error;
    }
  ...
  async function creerFichierAndroidFiable(parent, nomSouhaite) {
    const existante = await lireEntreeAndroid(parent, nomSouhaite);
    if (existante) {
      if (existante.kind !== "file") {
        throw new Error(`L’entrée « ${nomSouhaite} » existe mais n’est pas un fichier.`);
      }
      return existante;
    }
    ...
    try {
      ...
      if (estErreurTypeEntreeAndroid(error)) {
        const apresErreur = await lireEntreeAndroid(parent, nomSouhaite);
        if (apresErreur?.kind === "file") return apresErreur;
        throw new Error(
          `Impossible de créer le fichier « ${nomSouhaite} » : une entrée d’un autre type utilise ce nom.`
        );
      }
      throw error;
    }
  ...
  async function copierDossierAndroidRecursive(
    source,
    destination,
    ancienNom,
    nouveauNom,
    cheminLogique = "",
    nomsRacineReserves = new Set(),
    niveau = 0
  ) {
    const nomsReserves = new Set();

    for await (const [nom, handle] of source.entries()) {
      const typeEntree = handle?.kind;
      if (typeEntree !== "file" && typeEntree !== "directory") {
        throw new Error(`Type d’entrée Android inconnu pour « ${nom} ».`);
      }

      const nomRenomme =
        typeEntree === "file"
          ? remplacerNomDansNomFichier(nom, ancienNom, nouveauNom)
          : nom;

      let nomDemande = nomRenomme;
      if (
        niveau === 0 &&
        typeEntree === "file" &&
        nomsRacineReserves.has(nomDemande)
      ) {
        let indexCollision = 2;
        let candidat = nomAlternatifAndroid(nomDemande, indexCollision++);
        while (
          nomsRacineReserves.has(candidat) ||
          (await lireEntreeAndroid(destination, candidat))
        ) {
          candidat = nomAlternatifAndroid(nomDemande, indexCollision++);
        }
        nomDemande = candidat;
      }

      const nomCible = await choisirNomCibleAndroid(
        destination,
        nomDemande,
        typeEntree,
        nomsReserves
      );
      const cheminEntree = cheminLogique ? `${cheminLogique}/${nom}` : nom;

      try {
        if (typeEntree === "directory") {
          const sousDossier = await creerSousDossierAndroidFiable(
            destination,
            nomCible
          );
          await copierDossierAndroidRecursive(
            handle,
            sousDossier,
            ancienNom,
            nouveauNom,
            cheminLogique ? `${cheminLogique}/${nomCible}` : nomCible,
            nomsRacineReserves,
            niveau + 1
          );
        } else {
          const fichierSource = await handle.getFile();
          const cible = await creerFichierAndroidFiable(destination, nomCible);
          const writable = await cible.createWritable();
          try {
            await writable.write(fichierSource);
          } finally {
        ...
        throw new Error(
          `Copie interrompue sur « ${cheminEntree} » : ${error?.message || error}. La visite source est conservée.`
        );
      }
    }
  }
  ...
  async function trouverDossierVisiteAndroidDansVoyage(
    ...
    for (const villePreferee of villesPreferees) {
      const parentPrefere = await lireEntreeAndroid(voyageHandle, villePreferee);
      if (!parentPrefere || parentPrefere.kind !== "directory") continue;
      const dossierPrefere = await lireEntreeAndroid(parentPrefere, nomNettoye);
      if (!dossierPrefere || dossierPrefere.kind !== "directory") continue;
      return {
    ...
    for await (const [nomParent, parentHandle] of voyageHandle.entries()) {
      ...
      const dossier = await lireEntreeAndroid(parentHandle, nomNettoye);
      if (!dossier || dossier.kind !== "directory") continue;
  ...
  async function obtenirDossierAndroid(parent, nom, options = {}) {
    ...
    if (!nomNettoye) {
      throw new Error("Nom de dossier vide ou invalide.");
    }

    // v42.0.10 : on inspecte d’abord l’entrée réelle. Certains fournisseurs
    // Android renvoient un TypeMismatchError générique même lorsque le chemin
    // existe sous forme de fichier. Cette lecture préalable permet d’éviter
    // l’appel ambigu à getDirectoryHandle().
    const entreeExistante = await lireEntreeAndroid(parent, nomNettoye);
    if (entreeExistante) {
      if (entreeExistante.kind !== "directory") {
        const erreur = new Error(
          `L’entrée « ${nomNettoye} » existe sous forme de fichier et non de dossier. Aucune donnée source n’a été supprimée.`
        );
        erreur.code = "PHOTOCARTEL_ENTREE_FICHIER";
        throw erreur;
      }
      return entreeExistante;
    }

    if (options?.create !== true) {
      const erreur = new Error(`Dossier introuvable : ${nomNettoye}`);
      erreur.name = "NotFoundError";
      throw erreur;
    }

    try {
      return await parent.getDirectoryHandle(nomNettoye, { create: true });
    } catch (error) {
      if (estErreurTypeEntreeAndroid(error)) {
        throw new Error(
          `Impossible de créer le dossier « ${nomNettoye} » : une entrée d’un autre type utilise déjà ce nom. Aucune donnée source n’a été supprimée.`
        );
      }
      throw error;
    }
  ...
  async function mesurerDossierAndroid(dossier) {
    let fichiers = 0;
    let octets = 0;
    let dossiers = 1;

    for await (const [, handle] of dossier.entries()) {
      if (handle.kind === "directory") {
        const mesure = await mesurerDossierAndroid(handle);
        fichiers += mesure.fichiers;
        octets += mesure.octets;
        dossiers += mesure.dossiers;
      } else {
        const fichier = await handle.getFile();
        fichiers += 1;
        octets += Number(fichier.size || 0);
      }
    }

    return { fichiers, octets, dossiers };
  }

  async function creerCategoriesVisiteAndroid(dossierVisite, typeVisite) {
    for (const categorie of categoriesPourTypeVisiteLocale(typeVisite)) {
      await obtenirDossierAndroid(dossierVisite, categorie, { create: true });
    }
  }
  ...
  async function modifierIdentiteVisiteAndroid(payload) {
    ...
    const sourceTrouvee = await trouverDossierVisiteAndroidDansVoyage(
      voyageHandle,
      payload.ancienNom,
    ...
    if (memeParent && memeNom) {
      await creerCategoriesVisiteAndroid(
        ancienDossier,
        payload.nouveauType
      );
      return {
    ...
    const entreeDestinationExistante = await lireEntreeAndroid(
      nouveauParent,
      nouveauNomNettoye
    );
    if (entreeDestinationExistante) {
      throw new Error(
        entreeDestinationExistante.kind === "directory"
          ? "Une visite porte déjà ce nom dans cette ville."
          : `Un fichier porte déjà le nom « ${nouveauNomNettoye} » dans cette ville. La modification est interrompue sans toucher à la visite source.`
      );
    }

    let nouveauDossier = null;
    let destinationCreeeParCetteOperation = false;
    try {
      nouveauDossier = await obtenirDossierAndroid(
        nouveauParent,
        nouveauNomNettoye,
        { create: true }
      );
      destinationCreeeParCetteOperation = true;

      const mesureSourceAvant = await mesurerDossierAndroid(ancienDossier);
      const categoriesReservees = new Set(
        categoriesPourTypeVisiteLocale(payload.nouveauType)
      );
      ...
      const mesureDestination = await mesurerDossierAndroid(nouveauDossier);
      if (
        mesureDestination.fichiers !== mesureSourceAvant.fichiers ||
        mesureDestination.octets !== mesureSourceAvant.octets
      ) {
        throw new Error(
          `Vérification de copie refusée : source ${mesureSourceAvant.fichiers} fichier(s) / ${mesureSourceAvant.octets} octet(s), destination ${mesureDestination.fichiers} fichier(s) / ${mesureDestination.octets} octet(s). La visite source est conservée.`
        );
      }

      // Publication validée : la source n’est supprimée qu’après comparaison
      // complète du nombre de fichiers et du volume copié.
      await ancienParent.removeEntry(ancienNomNettoye, { recursive: true });
    } catch (error) {
      if (destinationCreeeParCetteOperation) {
        try {
          await nouveauParent.removeEntry(nouveauNomNettoye, { recursive: true });
        } catch (nettoyageError) {
          console.warn(
            "Nettoyage du dossier destination incomplet impossible :",
            nettoyageError?.message || nettoyageError
          );
        }
      }
      throw error;
  ...
  async function enregistrerModificationIdentiteVisite() {
    ...
    const ancienne = visiteEnModification;
    if (!ancienne) return;
  ...
  async function verifierCopieAndroid({ dossierDestination, nomDestination, fichierSource }) {
    const destinationHandle = await dossierDestination.getFileHandle(nomDestination, { create: false });
    const fichierDestination = await destinationHandle.getFile();

    if (fichierDestination.size !== fichierSource.size) {
      throw new Error(
        `Copie incomplète : ${fichierDestination.size} octet(s) au lieu de ${fichierSource.size}.`
      );
    }

    const [empreinteSource, empreinteDestination] = await Promise.all([
      empreinteSha256BlobAndroid(fichierSource),
      empreinteSha256BlobAndroid(fichierDestination),
    ]);

    if (empreinteSource !== empreinteDestination) {
  ...
  async function rangerPhotosVisitesAndroid(visitesAtraiter) {
    ...
    return {
      success: true,
      rangementComplet: photosEchecs === 0,
      mode: "android",
      photosLues,
      photosRangees,
      photosEchecs,
  ...
  function clePhysiqueVisite(visite = {}) {
    ...
    return [visite.voyage, stockageVille, visite.nom]
  ...
  function fusionnerVisitesPhysiquesEtMetadonnees(visitesDisque = []) {
    ...
    return visitesDisque.map((physique) => {
      ...
      return {
        ...
        statut: metadata ? (metadata.statut || (metadata.rangee ? "Rangée" : "Importée")) : "Importée",
        dateTriMs: Number(
          physique.datePhotoPlusRecenteMs || metadata?.dateRangementMs || metadata?.finMs || 0
        ),
      };
    });
  }
  ...
  async function analyserDossierVisiteAndroid(dossierVisite) {
    ...
    const parcourir = async (dossier) => {
      for await (const [nom, handle] of dossier.entries()) {
        ...
        nombrePhotos += 1;
        const fichier = await handle.getFile();
        const dateMs = extraireMsDepuisNomPhotoCartel(nom) || Number(fichier.lastModified || 0);
        if (dateMs) {
    ...
    for await (const [nomVoyage, voyageHandle] of voyages.entries()) {
      ...
      for await (const [nomVilleStockage, villeHandle] of voyageHandle.entries()) {
        ...
        for await (const [nomVisite, visiteHandle] of villeHandle.entries()) {
          if (visiteHandle.kind !== "directory") continue;
          const mesures = await analyserDossierVisiteAndroid(visiteHandle);
          const estVisiteRapide = nomVilleStockage === "Visites rapides";
          visites.push({
            idPhysique: [nomVoyage, nomVilleStockage, nomVisite].join("__"),
            voyage: nomVoyage,
  ...
  async function actualiserVisitesPhysiques({ autoriserSelection = false } = {}) {
    try {
      setLectureVisitesPhysiquesEnCours(true);
      ...
      // v42.0.8 : une erreur de synchronisation ne détruit jamais la liste
      // déjà disponible en mémoire ni la visite actuellement sélectionnée.
      // Une annulation du sélecteur reçoit un message PhotoCartel compréhensible.
  ...
  useEffect(() => {
    actualiserVisitesPhysiques({ autoriserSelection: false });
  }, []);

  function visitesRangeesRecentes() {
    return [...visitesPhysiques]
      .sort((a, b) => {
  ...
  useEffect(() => {
    ...
    const positionnerListe = (comportement = "smooth") => {
      ...
      const liste = document.getElementById("liste-dernieres-visites-rangees");
...
const validerNouvelleVisite = async () => {
  ...
  const baseStyles = {
    ...
    accueilInformations: {
      ...
      height: "100%",
      boxSizing: "border-box",
      padding: "66px 8px 7px 13px",
      background: "#fff8eb",
      display: "grid",
      gridTemplateRows: "repeat(3, minmax(0, 1fr)) 34px",
      textAlign: "left",
    },
    accueilPhoto: {
      width: "100%",
  ...
  function InfoLigne({ icone, label, valeur, action }) {
    ...
    return (
      <div className="photocartel-ligne-etat" style={styles.ligneEtat}>
      ...
      <section style={styles.resumeVisiteCarte} aria-label="Résumé de visite">
        <div style={styles.resumeVisiteTitreLigne}>
          ...
          <button
            ...
            style={{ ...styles.boutonModifierIdentite, position: "absolute", right: "1px", top: 0, width: "27px", height: "27px", fontSize: "15px" }}
        ...
        <div style={styles.resumeVisiteBoutonsSecondaires}>
          <button
            type="button"
            onClick={() => afficherMessageFonctionnaliteAccueil("Fonctionnalité bientôt disponible")}
        ...
        <button type="button" disabled style={styles.resumeVisiteBoutonActions}>
          ⋯ Actions
        </button>
      </section>
    );
  ...
  const pageAccueilPrincipaleAffichee =
    !resultatClassification && !dashboardRenommage && !ecranDernieresVisitesActif;
      ...
      <style>{`
        ...
        @media (max-width: 390px) {
          ...
          .photocartel-accueil-informations {
            padding: 62px 6px 6px 11px !important;
            grid-template-rows: repeat(3, minmax(0, 1fr)) 32px !important;
          }
      ...
      {modeParametres && (
        <div style={styles.modalOverlay}>
          <div style={styles.modal}>
            ...
            <section style={styles.parametresSection}>
              <div style={styles.sectionTitre}>Diagnostic stockage PWA — temporaire v39</div>
              <p style={styles.parametresTexte}>
                Ce test oublie volontairement les autorisations mémorisées par PhotoCartel sans fermer ni modifier la visite en cours.
              </p>
              <button
                type="button"
                ...
                style={{
                  ...styles.boutonAnalyseFermer,
                  marginTop: 9,
                  opacity: simulationPerteStockageV39EnCours || !estAndroid() ? 0.45 : 1,
                }}
              >
                {simulationPerteStockageV39EnCours
                  ? "Simulation en cours…"
                  : "🧪 Simuler une perte d’autorisation"}
              </button>
              {diagnosticStockageV39 && (
                <p style={{ ...styles.parametresChemin, whiteSpace: "pre-wrap" }}>
                  {diagnosticStockageV39}
                </p>
              )}
            </section>
          ...
          <>
            ...
            >
              ...
              <button
                type="button"
                onClick={() => {
                  // v42.0.7 : l’ouverture du menu exploite exclusivement la liste
                  // déjà préchargée en mémoire. Elle ne lit pas le stockage et ne
                  // déclenche jamais showDirectoryPicker().
              ...
              >
                <span>
                  {visiteResumeSelectionnee
                    ...
                    : "Sélectionner une visite récente rangée"}
                </span>
              ...
              {listeDernieresVisitesOuverte && (
                <div id="liste-dernieres-visites-rangees" className="photocartel-liste-dernieres-visites" style={styles.dernieresVisitesListe}>
                  <div style={styles.dernieresVisitesListeTitre}>Sélectionner une visite</div>
                  {lectureVisitesPhysiquesEnCours ? (
                    <div style={styles.dernieresVisitesVide}>Lecture des dossiers PhotoCartel…</div>
                        ...
                        return (
                          ...
                          >
                            <span style={styles.dernieresVisitesOptionIcone}>
                              {visite.statut === "Importée" ? "📥" : (visite.type ? "🏛️" : "⚡")}
                            </span>
