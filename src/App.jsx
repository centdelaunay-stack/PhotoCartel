...
const VERSION = {
  ...
  descriptif: "Galerie : photo absente signalée au lieu d'une image cassée",
  date: "2026-08-03",
  build: 5,
};
...
const [modeRecherches, setModeRecherches] = useState(false);
// v75 — moteur de recherche (photos et visites)
const [modeRechercheResultats, setModeRechercheResultats] = useState(false);
const [rechercheSaisie, setRechercheSaisie] = useState("");
const [rechercheCible, setRechercheCible] = useState("photos");
const [rechercheAnalyseesSeulement, setRechercheAnalyseesSeulement] = useState(false);
...
const [rechercheCriteresAffiches, setRechercheCriteresAffiches] = useState("");
const [rechercheStats, setRechercheStats] = useState({
  total: 0,
  analysees: 0,
  visites: 0,
  dureeMs: 0,
});
const [rechercheNombreVignettes, setRechercheNombreVignettes] = useState(24);
const [rechercheVignettes, setRechercheVignettes] = useState({});
const indexRechercheRef = useRef(null);
const rechercheVignettesRef = useRef({});
...
// ————————————————————————————————————————————————————————————————————————
// v75 — CACHE NAVIGATEUR DE L'INDEX DE RECHERCHE (IndexedDB)
//
// Même dispositif que celui de la galerie, et pour la même raison : le fichier
// d'index posé dans le dossier reste la source qui voyage entre le téléphone et
// C:, mais l'ouvrir demande un accès au système de fichiers Android, lent au
// premier accès d'une session. Une copie est donc gardée dans le navigateur.
//
// Base et store séparés de ceux de la galerie : les seize fonctions de cache
...
const CLE_CACHE_RECHERCHE = "index-recherche";
const VERSION_FORMAT_CACHE_RECHERCHE = 1;
...
function baseIndexedDbRecherche() {
  return new Promise((resolve) => {
    ...
    try {
      const requete = window.indexedDB.open(NOM_BASE_CACHE_RECHERCHE, 1);
...
async function lireCacheRechercheNavigateur() {
  try {
    ...
    const contenu = await new Promise((resolve) => {
      try {
        const requete = base
          .transaction(NOM_STORE_CACHE_RECHERCHE, "readonly")
          .objectStore(NOM_STORE_CACHE_RECHERCHE)
          .get(CLE_CACHE_RECHERCHE);
        requete.onsuccess = () => resolve(requete.result || null);
        requete.onerror = () => resolve(null);
      } catch (error) {
        resolve(null);
    ...
    if (!contenu || typeof contenu !== "object") return null;
    if (Number(contenu.version_format_cache) !== VERSION_FORMAT_CACHE_RECHERCHE) return null;

    return lireEntreesDepuisContenuIndexRecherche(contenu.index);
  } catch (error) {
    return null;
  }
}

async function ecrireCacheRechercheNavigateur(contenuIndex) {
  try {
    const base = await baseIndexedDbRecherche();
    if (!base) return false;

    const contenu = {
      version_format_cache: VERSION_FORMAT_CACHE_RECHERCHE,
      version_photocartel: VERSION_PHOTOCARTEL,
      date_cache_iso: new Date().toISOString(),
      index: contenuIndex,
    };
    ...
    return await new Promise((resolve) => {
      try {
        const requete = base
          .transaction(NOM_STORE_CACHE_RECHERCHE, "readwrite")
          .objectStore(NOM_STORE_CACHE_RECHERCHE)
          .put(contenu, CLE_CACHE_RECHERCHE);
        requete.onsuccess = () => resolve(true);
        requete.onerror = () => resolve(false);
      } catch (error) {
        resolve(false);
    ...
    // Le cache est une optimisation : son échec ne doit jamais bloquer la
    // recherche, qui retombe alors sur le fichier d'index.
    return false;
  }
}

async function lireIndexRechercheAndroid(dossierPhotosAnalysees) {
  try {
    const handle = await dossierPhotosAnalysees.getFileHandle(
      NOM_FICHIER_INDEX_RECHERCHE
    );
    const fichier = await handle.getFile();
    return lireEntreesDepuisContenuIndexRecherche(JSON.parse(await fichier.text()));
  } catch (error) {
    return null;
  }
}

async function ecrireIndexRechercheAndroid(dossierPhotosAnalysees, contenuIndex) {
  try {
    await ecrireBlobDansDossierAndroid(
      dossierPhotosAnalysees,
      NOM_FICHIER_INDEX_RECHERCHE,
      new Blob([JSON.stringify(contenuIndex)], { type: "application/json" })
    );
    return true;
  } catch (error) {
...
// Parcourt Voyages/<voyage>/<ville>/<visite> et « Visites à rattacher » et ne
// LISTE QUE DES NOMS. Aucun fichier n'est ouvert : c'est le levier de coût déjà
// établi pour la galerie, et le seul qui rende ce parcours supportable sur des
// milliers de photos.
async function listerPhotosVisitesAndroid() {
  const resultatRacine = await obtenirDossierRacinePhotoCartelAndroid({
    ouvrirSelecteurSiNecessaire: false,
    demanderPermissionSiNecessaire: true,
  });
  ...
  const listerDossierVisite = async (handleVisite, nomVisite, nomVoyage, nomVille) => {
    for await (const [nomFichier, handleFichier] of handleVisite.entries()) {
      if (handleFichier.kind !== "file") continue;
      if (!estFichierImageRecherche(nomFichier)) continue;
      photos.push({
        nomPhoto: nomFichier,
        nomJson: "",
        visite: nomVisite,
  ...
  try {
    const voyages = await racine.getDirectoryHandle(DOSSIER_METIER_VOYAGES, {
      create: true,
    });
    for await (const [nomVoyage, handleVoyage] of voyages.entries()) {
      if (handleVoyage.kind !== "directory") continue;
      for await (const [nomVille, handleVille] of handleVoyage.entries()) {
        if (handleVille.kind !== "directory") continue;
        for await (const [nomVisite, handleVisite] of handleVille.entries()) {
          if (handleVisite.kind !== "directory") continue;
          if (estNomTechniquePhotoCartel(nomVisite)) continue;
          await listerDossierVisite(handleVisite, nomVisite, nomVoyage, nomVille);
        }
      }
    }
  } catch (error) {
    console.warn("Parcours des voyages impossible pour l’index de recherche :", error);
  }

  try {
    const aRattacher = await racine.getDirectoryHandle(DOSSIER_VISITES_A_RATTACHER, {
      create: true,
    });
    for await (const [nomVisite, handleVisite] of aRattacher.entries()) {
      if (handleVisite.kind !== "directory") continue;
      if (estNomTechniquePhotoCartel(nomVisite)) continue;
      await listerDossierVisite(handleVisite, nomVisite, "", "");
    }
  } catch (error) {
    console.warn("Parcours des visites à rattacher impossible :", error);
  }

  return photos;
}

// Construction complète de l'index de recherche côté Android. Faite UNE FOIS :
// ensuite l'index est relu depuis le cache navigateur ou depuis le fichier, et
// tenu à jour à chaque analyse enregistrée.
async function construireIndexRechercheAndroid() {
  const dossierPhotosAnalysees = await obtenirDossierPhotosAnalyseesAndroid({
    verifierInfrastructure: false,
  });

  const { fiches } = await lireToutesFichesGalerieAndroid(dossierPhotosAnalysees);
  const photosVisites = await listerPhotosVisitesAndroid();

  // Rattachement d'une fiche à sa visite par comparaison de NOMS, la même
  // technique bon marché que la resynchronisation de la galerie : le nom de la
  // photo d'origine conservé dans la fiche est cherché parmi les noms listés
  // dans les dossiers de visite. Aucun fichier supplémentaire n'est ouvert.
  const visiteParNomPhoto = new Map();
  for (let i = 0; i < photosVisites.length; i += 1) {
    if (!visiteParNomPhoto.has(photosVisites[i].nomPhoto)) {
      visiteParNomPhoto.set(photosVisites[i].nomPhoto, photosVisites[i]);
    }
  }
  ...
  // Une photo de visite déjà analysée n'est pas indexée deux fois : c'est sa
  // fiche, porteuse du texte cherchable, qui la représente.
  for (let i = 0; i < photosVisites.length; i += 1) {
    if (nomsPhotosAnalysees.has(photosVisites[i].nomPhoto)) continue;
    sources.push(photosVisites[i]);
  }

  const contenuIndex = construireContenuIndexRecherche(sources, VERSION_PHOTOCARTEL);
  await ecrireIndexRechercheAndroid(dossierPhotosAnalysees, contenuIndex);
  await ecrireCacheRechercheNavigateur(contenuIndex);

  return lireEntreesDepuisContenuIndexRecherche(contenuIndex);
}

// Ordre d'obtention, du moins cher au plus cher : cache navigateur (aucun accès
// au dossier), puis fichier d'index, puis construction complète.
async function obtenirIndexRecherche({ forcerReconstruction = false } = {}) {
  if (!estAndroid()) {
    const reponse = await fetch(
      API_BASE +
        "/index-recherche?dossierRacine=" +
        encodeURIComponent(dossierRacineGalerieEnvoyeAuServeur()) +
        (forcerReconstruction ? "&reconstruire=1" : "")
    );
    const data = await lireReponseJsonPhotoCartel(
      reponse,
      "Erreur construction de l’index de recherche"
    );
    if (!reponse.ok || !data.success) {
      throw new Error(data.error || "Erreur construction de l’index de recherche");
    }
    return lireEntreesDepuisContenuIndexRecherche(data.index);
  }

  if (!forcerReconstruction) {
    const depuisCache = await lireCacheRechercheNavigateur();
    ...
    try {
      ...
      const depuisFichier = await lireIndexRechercheAndroid(dossierPhotosAnalysees);
      if (depuisFichier) return depuisFichier;
    } catch (error) {
      // Dossier indisponible : on retombe sur la construction complète, qui
      // redemandera la permission.
    }
  }

  return construireIndexRechercheAndroid();
}

// ————————————————————————————————————————————————————————————————————————
// v75 — MOTEUR DE RECHERCHE (photos et visites)
//
// Le filtrage porte sur l'index de recherche, déjà en mémoire et déjà
// normalisé : il ne touche jamais au disque et n'ouvre aucune image. Les
// vignettes ne sont chargées que par lots visibles, et seulement dans la vue
// grille — une recherche qui ramène plusieurs centaines de photos ne doit pas
// se transformer en autant d'ouvertures de fichiers, qui étaient exactement le
// défaut de la galerie en v68.
// ————————————————————————————————————————————————————————————————————————

// Resynchronisation de l'index de recherche avec la galerie, par comparaison de
// NOMS : les fiches présentes dans la galerie mais absentes de l'index y sont
// ajoutées, sans aucun accès au dossier. C'est ce qui fait qu'une analyse
// enregistrée depuis la dernière recherche est trouvable sans reconstruire
// l'index, et sans toucher au chemin d'enregistrement.
function resynchroniserIndexRechercheDepuisGalerie(index, fichesGalerie) {
  if (!index || !Array.isArray(fichesGalerie) || fichesGalerie.length === 0) {
    return index;
  }

  const nomsIndexes = new Set(
    index.photos.filter((entree) => entree.analysee).map((entree) => entree.nomPhoto)
  );

  const ajouts = [];
  for (let i = 0; i < fichesGalerie.length; i += 1) {
    const fiche = fichesGalerie[i];
    if (!fiche?.nomPhoto || nomsIndexes.has(fiche.nomPhoto)) continue;
    ajouts.push(
      construireEntreeRecherchePhoto({
        nomPhoto: fiche.nomPhoto,
        nomJson: fiche.nomJson,
  ...
  if (ajouts.length === 0) return index;

  const photos = index.photos.concat(ajouts);
  return { photos, visites: construireEntreesRechercheVisites(photos) };
}

async function ouvrirEcranRecherche() {
  setModeRecherches(false);
  setMessageMenuAccueil("");
  setCibleMessageMenuAccueil("");
  setModeRechercheResultats(true);
  setRechercheResultats([]);
  setRechercheCriteresAffiches("");
  ...
  try {
    ...
    setRechercheMessage("Préparation de l’index de recherche...");
    let index = await obtenirIndexRecherche();
    // Une analyse enregistrée depuis la dernière recherche n'est pas encore dans
    // l'index : elle y est ajoutée par comparaison de noms avec la galerie déjà
    // en mémoire, sans aucun accès au dossier.
    const fichesGalerie =
      galerieAnalysesCacheRef.current.length > 0
        ? galerieAnalysesCacheRef.current
        : await chargerGalerieDepuisCacheNavigateur();
    index = resynchroniserIndexRechercheDepuisGalerie(index, fichesGalerie);
    indexRechercheRef.current = index;
    setRechercheMessage(
      index
        ? `${index.photos.length} photo(s) indexée(s), dont ${
            index.photos.filter((entree) => entree.analysee).length
          } analysée(s), sur ${index.visites.length} visite(s).`
        : "Index de recherche indisponible."
    );
  } catch (error) {
    console.error("Index de recherche indisponible :", error);
    setRechercheMessage("Index de recherche indisponible : " + error.message);
  } finally {
    setRechercheChargement(false);
  }
}

async function reconstruireIndexRecherche() {
  try {
    setRechercheChargement(true);
    setRechercheMessage("Reconstruction de l’index de recherche...");
    const index = await obtenirIndexRecherche({ forcerReconstruction: true });
    indexRechercheRef.current = index;
    setRechercheResultats([]);
    setRechercheCriteresAffiches("");
    setRechercheMessage(
      index
        ? `Index reconstruit : ${index.photos.length} photo(s), ${index.visites.length} visite(s).`
        : "Index de recherche indisponible."
    );
  } catch (error) {
    console.error("Reconstruction de l’index impossible :", error);
    setRechercheMessage("Reconstruction impossible : " + error.message);
  } finally {
    setRechercheChargement(false);
  }
}

function lancerRechercheMotsCles() {
  const index = indexRechercheRef.current;
  if (!index) {
    setRechercheMessage("L’index de recherche n’est pas encore prêt.");
    return;
  }

  const mots = decouperMotsClesRecherche(rechercheSaisie);
  const criteres = {
    mots,
    analyseesSeulement: rechercheAnalyseesSeulement,
  };

  const debut = Date.now();
  const entrees = rechercheCible === "visites" ? index.visites : index.photos;
  const resultats = filtrerEntreesRecherche(entrees, criteres);
  const duree = Date.now() - debut;

  setRechercheResultats(resultats);
  setRechercheNombreVignettes(24);
  setRechercheVignettes({});
  setRechercheVue("tableau");
  setRechercheStats({
    total: index.photos.length,
    analysees: index.photos.filter((entree) => entree.analysee).length,
    visites: index.visites.length,
    dureeMs: duree,
  });
  setRechercheCriteresAffiches(
    mots.length > 0
      ? `Mots-clés : ${mots.join(" + ")}${
          rechercheAnalyseesSeulement ? " · photos analysées seulement" : ""
        }`
      : rechercheAnalyseesSeulement
        ? "Photos analysées seulement"
        : "Aucun critère — tout est retenu"
  );
  setRechercheMessage("");
}

// Chargement d'une vignette, uniquement pour une photo réellement affichée dans
// la grille. Sur Android l'image vient du dossier « Photos analysées » ; une
// photo de visite jamais analysée n'a pas de vignette ici, son dossier n'étant
// pas ouvert par la recherche.
async function chargerVignetteRechercheAndroid(nomPhoto) {
  const nom = String(nomPhoto || "");
  if (!estAndroid() || !nom) return;
  if (rechercheVignettesRef.current[nom] !== undefined) return;

  rechercheVignettesRef.current = { ...rechercheVignettesRef.current, [nom]: "" };
  ...
  try {
    ...
    const handle = await dossierPhotosAnalysees.getFileHandle(nom);
    const url = URL.createObjectURL(await handle.getFile());
    rechercheVignettesRef.current = { ...rechercheVignettesRef.current, [nom]: url };
    setRechercheVignettes(rechercheVignettesRef.current);
  } catch (error) {
    dossierPhotosAnalyseesAndroidRef.current = null;
    setRechercheVignettes(rechercheVignettesRef.current);
  }
...
async function lancerAnalyseRenommage() {
  ...
  try {
    ...
    const response = await fetch(API_BASE + "/renommer-oeuvres/analyser", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
...
const validerNouvelleVisite = async () => {
  ...
  const pageAccueilPrincipaleAffichee =
    !resultatClassification && !dashboardRenommage && !ecranDernieresVisitesActif && !modeGalerieVisite;
  ...
  return (
    ...
    >
      ...
      {modeRechercheResultats && (() => {
        // Styles littéraux : les deux jeux de styles de l'application (base et
        // thème) sont construits clé par clé, une clé ajoutée à l'un seulement
        // serait indéfinie sur l'autre.
        const styleEnteteRecherche = {
          textAlign: "left",
          borderBottom: "1px solid #d4d4d4",
          padding: "6px 4px",
          fontWeight: 700,
        };
        const styleCelluleRecherche = {
        ...
        <div style={{ ...styles.analyseEcran, paddingBottom: "154px" }}>
          <main style={{ ...styles.analyseTelephone, paddingBottom: "148px" }}>
            <h1 style={styles.titreFicheResultat}>Recherche</h1>

            <div style={styles.analyseCarte}>
              <input
                type="text"
                value={rechercheSaisie}
                onChange={(evenement) => setRechercheSaisie(evenement.target.value)}
                placeholder="Ex. : peintures de Rembrandt"
                style={{
                  width: "100%",
                  boxSizing: "border-box",
                  padding: "10px 12px",
              ...
              <div style={{ display: "flex", gap: "8px", marginTop: "10px", flexWrap: "wrap" }}>
                <button
                  type="button"
                  onClick={() => setRechercheCible("photos")}
                  style={{
                    flex: "1 1 120px",
                    padding: "8px 10px",
                    borderRadius: "8px",
                    border: "1px solid #c9c9c9",
                    background: rechercheCible === "photos" ? "#1f6feb" : "#ffffff",
                    color: rechercheCible === "photos" ? "#ffffff" : "#222222",
                  }}
                >
                  Par photo
                </button>
                <button
                  type="button"
                  onClick={() => setRechercheCible("visites")}
                  style={{
                    flex: "1 1 120px",
                    padding: "8px 10px",
                    borderRadius: "8px",
                    border: "1px solid #c9c9c9",
                    background: rechercheCible === "visites" ? "#1f6feb" : "#ffffff",
                    color: rechercheCible === "visites" ? "#ffffff" : "#222222",
                  }}
                >
                  Par visite
                </button>
              ...
              <label style={{ display: "flex", alignItems: "center", gap: "8px", marginTop: "10px" }}>
                <input
                  type="checkbox"
                  checked={rechercheAnalyseesSeulement}
                  onChange={(evenement) =>
                    setRechercheAnalyseesSeulement(evenement.target.checked)
                  }
                />
                Photos analysées seulement
              </label>
              ...
              <div style={{ display: "flex", gap: "8px", marginTop: "12px", flexWrap: "wrap" }}>
                <button
                  type="button"
                  onClick={lancerRechercheMotsCles}
                  disabled={rechercheChargement}
                  style={{
                    flex: "1 1 140px",
                    padding: "10px",
                    borderRadius: "8px",
                ...
                <button
                  type="button"
                  onClick={reconstruireIndexRecherche}
                  disabled={rechercheChargement}
                  style={{
                    flex: "1 1 140px",
                    padding: "10px",
                ...
                <button
                  type="button"
                  onClick={() => setModeRechercheResultats(false)}
                  style={{
                    flex: "1 1 100px",
                    padding: "10px",
                    borderRadius: "8px",
            ...
            {rechercheMessage && (
              <div style={styles.analyseCarte}>
                <p style={{ margin: 0 }}>{rechercheMessage}</p>
              </div>
            )}

            {rechercheCriteresAffiches && (
              <>
                <div style={styles.analyseCarte}>
                  <h2 style={styles.analyseType}>Critères de recherche</h2>
                  <p style={{ margin: 0 }}>{rechercheCriteresAffiches}</p>
                </div>

                <div style={styles.analyseCarte}>
                  <h2 style={styles.analyseType}>
                    {rechercheResultats.length}{" "}
                    {rechercheCible === "visites" ? "visite(s)" : "photo(s)"} trouvée(s)
                  </h2>
                  {/* Le dénominateur est affiché en clair : sans lui, un résultat
                      vide se lit comme un défaut alors qu'il ne fait souvent que
                      constater qu'une photo n'a jamais été analysée, donc ne porte
                      aucun texte à chercher. */}
                  <p style={{ margin: 0 }}>
                    {rechercheCible === "visites"
                      ? `sur ${rechercheStats.visites} visite(s) indexée(s)`
                      : `sur ${rechercheStats.total} photo(s) indexée(s), dont ${rechercheStats.analysees} analysée(s) — ${rechercheStats.total - rechercheStats.analysees} photo(s) sans analyse ne portent aucun texte à chercher`}
                  </p>
                  <p style={{ margin: "6px 0 0" }}>Recherche exécutée en {rechercheStats.dureeMs} ms.</p>
                  {rechercheCible === "photos" && rechercheResultats.length > 0 && (
                    <button
                      type="button"
                      onClick={() =>
                        setRechercheVue(rechercheVue === "grille" ? "tableau" : "grille")
                      }
                      style={{
                        marginTop: "10px",
                        padding: "8px 12px",
                    ...
                    >
                      {rechercheVue === "grille" ? "Revenir au tableau" : "Grille des photos"}
                    </button>
                  )}
                </div>
              </>
            ...
            {rechercheCriteresAffiches && rechercheVue === "tableau" && (
              <div style={{ ...styles.analyseCarte, overflowX: "auto" }}>
                  ...
                  <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
                    <thead>
                      <tr>
                        {rechercheCible === "visites" ? (
                          <>
                            <th style={styleEnteteRecherche}>Visite</th>
                            <th style={styleEnteteRecherche}>Voyage</th>
                            <th style={styleEnteteRecherche}>Photos</th>
                            <th style={styleEnteteRecherche}>Analysées</th>
                          </>
                        ) : (
                          <>
                            <th style={styleEnteteRecherche}>Titre</th>
                            <th style={styleEnteteRecherche}>Auteur</th>
                            <th style={styleEnteteRecherche}>Institution</th>
                            <th style={styleEnteteRecherche}>Visite</th>
                          </>
                        )}
                      </tr>
                    ...
                    <tbody>
                      {rechercheResultats.slice(0, 500).map((entree, position) => (
                        <tr key={`${entree.nomPhoto || entree.visite}-${position}`}>
                          {rechercheCible === "visites" ? (
                            <>
                              <td style={styleCelluleRecherche}>{entree.visite || "—"}</td>
                              <td style={styleCelluleRecherche}>{entree.voyage || "—"}</td>
                              <td style={styleCelluleRecherche}>{entree.nombrePhotos}</td>
                              <td style={styleCelluleRecherche}>{entree.nombreAnalysees}</td>
                            </>
                          ) : (
                            <>
                              <td style={styleCelluleRecherche}>
                                {entree.titre || entree.nomPhoto || "—"}
                              </td>
                              <td style={styleCelluleRecherche}>{entree.auteur || "—"}</td>
                              <td style={styleCelluleRecherche}>{entree.institution || "—"}</td>
                              <td style={styleCelluleRecherche}>{entree.visite || "—"}</td>
                            </>
                          )}
                        </tr>
                ...
                {rechercheResultats.length > 500 && (
                  <p style={{ margin: "8px 0 0" }}>
                    500 premières lignes affichées sur {rechercheResultats.length}. Affine les
                    mots-clés pour resserrer le résultat.
                  </p>
                )}
              </div>
            )}
            ...
            {rechercheCriteresAffiches &&
              ...
              rechercheCible === "photos" && (
                <div style={styles.analyseCarte}>
                  ...
                  >
                    {rechercheResultats
                      .slice(0, rechercheNombreVignettes)
                      .map((entree, position) => {
                        const url = estAndroid()
                          ? rechercheVignettes[entree.nomPhoto] || ""
                          : entree.analysee
                            ? urlPhotoGalerie({
                                imageUrlLocale: false,
                            ...
                            : "";
                        if (estAndroid() && entree.analysee && !url) {
                          chargerVignetteRechercheAndroid(entree.nomPhoto);
                        }
                        return (
                          <div
                            key={`${entree.nomPhoto}-${position}`}
                            style={{
                              aspectRatio: "1 / 1",
                              background: "#eeeeee",
                              borderRadius: "6px",
                          ...
                          >
                            {url ? (
                              <img
                                src={url}
                                alt={entree.titre || entree.nomPhoto}
                                loading="lazy"
                                style={{ width: "100%", height: "100%", objectFit: "cover" }}
                              />
                            ) : (
                              <span>{entree.analysee ? "…" : "Non analysée"}</span>
                            )}
                          </div>
                        );
                      })}
                  </div>
                  {rechercheResultats.length > rechercheNombreVignettes && (
                    <button
                      type="button"
                      onClick={() => setRechercheNombreVignettes(rechercheNombreVignettes + 24)}
                      style={{
                        marginTop: "10px",
                        width: "100%",
                        padding: "10px",
                    ...
                    >
                      Afficher 24 photos de plus ({rechercheNombreVignettes} sur{" "}
                      {rechercheResultats.length})
                    </button>
                  )}
                </div>
      ...
      {modeBibliotheques && (
        <div style={styles.modalOverlay}>
          <div style={styles.modal}>
            ...
            <BoutonMenuPopup icone="🔎" titre="Rechercher" onClick={() => {
              setModeBibliotheques(false);
              setMessageMenuAccueil("");
              setCibleMessageMenuAccueil("");
              setModeRecherches(true);
            }} />
            <BoutonMenuPopup titre="Fermer" secondaire onClick={() => {
              setModeBibliotheques(false);
              setMessageMenuAccueil("");
      ...
      {modeRecherches && (
        <div style={styles.modalOverlay}>
          <div style={styles.modal}>
            <h3 style={{ margin: "0 0 12px" }}>Recherches</h3>
            <BoutonMenuPopup
              icone="🔎"
              titre="Recherche par critères"
              onClick={ouvrirEcranRecherche}
            />
            <BoutonMenuPopup
              icone="💬"
              titre="Recherche par langage naturel"
              onClick={() => afficherMessageSousBouton("recherches-langage-naturel")}
            />
            <MessageSousBouton cible="recherches-langage-naturel" />
            <BoutonMenuPopup
              icone="✨"
              titre="Recherche intelligente"
              onClick={() => afficherMessageSousBouton("recherches-intelligente")}
            />
            <MessageSousBouton cible="recherches-intelligente" />
            <BoutonMenuPopup titre="Fermer" secondaire onClick={() => {
              setModeRecherches(false);
              setMessageMenuAccueil("");
              setCibleMessageMenuAccueil("");
