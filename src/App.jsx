// PhotoCartel v45.6 — rangement Android/PWA vérifié avec relecture robuste sans dépendance obligatoire à Web Crypto.
...
const VERSION = {
  ...
  descriptif: "Accueil compact et drapeaux de voyage",
  date: "2026-07-26",
  build: 1,
};
...
// En l'absence d'un pays identifiable, le drapeau européen est affiché.
...
const TAILLE_INITIALE_GALERIE_VISITE = 30;
const TAILLE_PAGE_FOND_GALERIE_VISITE = 500;
const COLONNES_GALERIE_VISITE = 3;
const SURBALAYAGE_LIGNES_GALERIE = 4;
const CONCURRENCE_PREFETCH_MINIATURES = 2;
...
const afficherMessageDiscretArborescence = (message, duree = 5000) => {
  ...
  async function empreinteSha256BlobAndroid(blob) {
    ...
    const empreinte = await subtle.digest("SHA-256", contenu);
    return Array.from(new Uint8Array(empreinte))
      .map((octet) => octet.toString(16).padStart(2, "0"))
      .join("");
  }

  async function comparerBlobsExactementAndroid(blobSource, blobDestination) {
    if (blobSource.size !== blobDestination.size) return false;

    const TAILLE_BLOC = 1024 * 1024;
    for (let debut = 0; debut < blobSource.size; debut += TAILLE_BLOC) {
      const fin = Math.min(blobSource.size, debut + TAILLE_BLOC);
      const [sourceBuffer, destinationBuffer] = await Promise.all([
        blobSource.slice(debut, fin).arrayBuffer(),
        blobDestination.slice(debut, fin).arrayBuffer(),
      ]);
      const source = new Uint8Array(sourceBuffer);
      const destination = new Uint8Array(destinationBuffer);
  ...
  async function verifierCopieAndroid({ dossierDestination, nomDestination, fichierSource }) {
    // Chrome Android/SAF peut rendre un File obsolète pendant quelques millisecondes
    // juste après close(). On relit donc le handle à chaque tentative.
    let derniereErreur = null;
    const delais = [0, 80, 180, 350, 700];
      ...
      try {
        ...
        if (empreinteSource && empreinteDestination) {
          ...
          return {
            ...
            methodeVerification: "sha256",
          };
        }

        // Sur une PWA servie hors contexte sécurisé, crypto.subtle peut être absent.
        // La vérification reste alors complète, octet par octet et par blocs bornés.
        const identiques = await comparerBlobsExactementAndroid(
          fichierSource,
          fichierDestination
        );
        if (!identiques) {
          throw new Error("La copie relue diffère du fichier source.");
        }

        return {
          taille: fichierDestination.size,
          empreinteSha256: null,
          methodeVerification: "comparaison-octets",
        };
      } catch (error) {
  ...
  async function ouvrirGalerieVisiteMaquette(visite) {
    ...
    try {
      ...
      if (estAndroid()) {
        ...
        setChargementGalerieVisite(false);
        prechargerMiniaturesGalerieEnArrierePlan(photos, generation);
      } else {
        ...
        setChargementGalerieVisite(false);
        prechargerMiniaturesGalerieEnArrierePlan(premiereTranche, generation);
        ...
        while (offsetSuivant !== null && offsetSuivant !== undefined && generationGalerieVisiteRef.current === generation) {
          ...
          prechargerMiniaturesGalerieEnArrierePlan(nouvelles, generation);
  ...
  function BarreGalerieVisite({ fichePhoto = false }) {
    const actions = fichePhoto
      ? [
          ...
          { icone: "🖼️", texte: "Grille", action: () => setModePhotoGalerieVisite(false) },
    ...
    return (
      <section style={styles.galerieVisiteEcran} aria-label="Page d’entrée de la Galerie d’une visite">
        ...
        {!erreurGalerieVisite && nombreTotalPhotosGalerieVisite > 0 && (() => {
          const largeurCellule = Math.max(80, largeurGalerieVisite / COLONNES_GALERIE_VISITE);
          const hauteurLigne = largeurCellule;
          const nombreLignes = Math.ceil(nombreTotalPhotosGalerieVisite / COLONNES_GALERIE_VISITE);
          const ligneDebut = Math.max(0, Math.floor(scrollTopGalerieVisite / hauteurLigne) - SURBALAYAGE_LIGNES_GALERIE);
          const ligneFin = Math.min(nombreLignes, Math.ceil((scrollTopGalerieVisite + hauteurGalerieVisite) / hauteurLigne) + SURBALAYAGE_LIGNES_GALERIE);
          const indexDebut = ligneDebut * COLONNES_GALERIE_VISITE;
          const indexFin = Math.min(nombreTotalPhotosGalerieVisite, ligneFin * COLONNES_GALERIE_VISITE);
          const cellules = [];
          for (let index = indexDebut; index < indexFin; index += 1) {
            const photo = photosGalerieVisite[index];
            const colonne = index % COLONNES_GALERIE_VISITE;
            const ligne = Math.floor(index / COLONNES_GALERIE_VISITE);
            cellules.push(
              <button
          ...
          return (
            <div
              ...
              onScroll={(event) => setScrollTopGalerieVisite(event.currentTarget.scrollTop)}
            >
              <div style={{ position: "relative", width: "100%", height: `${nombreLignes * hauteurLigne}px` }}>
                {cellules}
              </div>
            </div>
          );
      ...
      {modeBibliotheques && (
        <div style={styles.modalOverlay}>
          <div style={styles.modal}>
            ...
            <BoutonMenuPopup icone="🔎" titre="Rechercher" onClick={() => afficherMessageSousBouton("bibliotheques-rechercher")} />
            <MessageSousBouton cible="bibliotheques-rechercher" />
            <BoutonMenuPopup titre="Fermer" secondaire onClick={() => {
              setModeBibliotheques(false);
              setMessageMenuAccueil("");
              setCibleMessageMenuAccueil("");
