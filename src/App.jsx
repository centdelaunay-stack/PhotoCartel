// PhotoCartel v40.5 — finition du résumé de visite et icônes de types cohérentes.
...
function App() {
  ...
  async function verifierServeurModificationVisite() {
    ...
    if (String(informations.version || "") !== "v40.5") {
      throw new Error(
        `Le serveur actif annonce ${informations.version || "une version inconnue"}, pas v40.5. ` +
...
const afficherMessageDiscretArborescence = (message, duree = 5000) => {
  ...
  async function trouverDossierVisiteAndroidDansVoyage(
    ...
    const villePreferee = nettoyerNomDossierLocal(villeStockagePreferee);

    // v40.4 : le chemin de stockage mémorisé est prioritaire et suffisant.
    // Une recherche globale n'est lancée que si ce chemin précis n'existe plus.
    if (villePreferee) {
      try {
    ...
    for await (const [nomParent, parentHandle] of voyageHandle.entries()) {
      ...
      if (villePreferee && nettoyerNomDossierLocal(nomParent) === villePreferee) continue;
      try {
...
const validerNouvelleVisite = async () => {
  ...
  const baseStyles = {
    ...
    resumeVisiteLignePrincipale: {
      ...
      borderTop: "none",
      borderRadius: "7px",
      background: "linear-gradient(90deg, rgba(255,253,248,0.96), rgba(238,224,199,0.62), rgba(255,253,248,0.96))",
    },
  ...
  function BlocDerniereVisite() {
    ...
    return (
      <section style={styles.carteDerniereVisite}>
        ...
        <DetailLigne icone={iconePourTypeVisite(derniereVisite.type || "Visite rapide")} label="Type de visite" valeur={derniereVisite.type} />
