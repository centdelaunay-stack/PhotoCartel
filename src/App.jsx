...
const VERSION = {
  numero: "v36",
  descriptif: "modification et enregistrement des analyses depuis la galerie",
  date: "2026-07-14",
  build: 1,
};
...
// PhotoCartel v36 — connexion de « Modifier l’analyse » et « Enregistrer l’analyse » dans la galerie.
// Une modification remplace le JSON existant sans renommer le JSON ni toucher à la photo.
...
async function modifierAnalyseGalerieAndroid({ fiche, analyse }) {
  ...
  const nomJson = String(fiche?.nomJson || "").trim();

  if (!nomJson) {
    throw new Error("Nom du fichier JSON manquant.");
  }

  const handleJson = await dossierPhotosAnalysees.getFileHandle(nomJson);
  ...
  const contenuModifie = {
    ...
    nom_photo_sauvegardee: fiche.nomPhoto || contenuExistant.nom_photo_sauvegardee || "",
    nom_json_sauvegarde: nomJson,
    analyse,
  };
  ...
  return {
    success: true,
    nomJson,
    nomPhoto: fiche.nomPhoto || contenuExistant.nom_photo_sauvegardee || "",
    dateAnalyseIso,
    dateAnalyseLocale,
  };
}
                ...
                <>
                  ...
                  <div style={styles.dateHeurePhotoCarte}>
                    <div style={styles.dateHeurePhotoLigne}>
                      <div style={styles.dateHeurePhotoLabel}>📷 Prise de vue :</div>
                      <div style={styles.dateHeurePhotoValeur}>
                        {fiche.datePhotoLocale || "Date non disponible pour cette ancienne fiche"}
                      </div>
                    </div>
                    <div style={styles.dateHeurePhotoLigne}>
                      <div style={styles.dateHeurePhotoLabel}>🤖 Analyse IA :</div>
                      <div style={styles.dateHeurePhotoValeur}>
                        {fiche.dateAnalyseLocale || "Date non disponible"}
                      </div>
                    </div>
                  </div>
