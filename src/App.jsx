...
const VERSION = {
  numero: "v35.3",
  descriptif: "chargement local corrigé de la galerie",
  date: "2026-07-13",
  build: 3,
};
...
// PhotoCartel v35.3 — correction du chargement local de la galerie depuis la racine officielle PhotoCartel.
...
async function ouvrirGaleriePhotosAnalysees() {
  // v35.2 : les analyses déjà chargées restent en mémoire pendant la session.
  // Un retour Accueil → Galerie réaffiche donc immédiatement la fiche courante,
  // sans grand écran « Chargement… » inutile.
...
function fermerGaleriePhotosAnalysees() {
  // v35.2 : on ferme uniquement l'écran. Le contenu reste en cache de session.
      ...
      >
        {actions.map((action) => (
          <button
            ...
            aria-label={`${action.texte} — non connecté dans la v35.2`}
            title={`${action.texte} — non connecté dans la v35.2`}
          >
            <span style={styles.barreActionsFicheIcone}>{action.icone}</span>
            <span style={styles.barreActionsFicheTexte}>{action.texte}</span>
          </button>
