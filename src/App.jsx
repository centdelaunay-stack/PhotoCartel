...
const VERSION = {
  numero: "v28",
  descriptif: "initialisation arborescence infrastructure PhotoCartel dans DCIM",
  date: "2026-07-07",
  build: 1,
};
...
// PhotoCartel v28 — initialisation des dossiers système/infrastructure dans DCIM/PhotoCartel.
...
async function creerDossiersNouvelleVisiteAndroid({ voyageNom, villeNom, visiteNom, typeVisiteNom }) {
  ...
  const dossierVoyage = await racinePhotoCartel.getDirectoryHandle(nomVoyage, {
    create: true,
  });
  ...
  return {
    success: true,
    cheminLisible:
      "DCIM / PhotoCartel / " + nomVoyage + " / " + nomVille + " / " + nomVisite,
...
async function creerArborescenceInfrastructurePhotoCartel(dossierDcim) {
  // v28 : point unique de création de l'arborescence système/infrastructure.
  // L'utilisateur sélectionne DCIM ; PhotoCartel crée ensuite DCIM/PhotoCartel
  // puis les dossiers système définis par la spécification d'arborescence.
  const dossierPhotoCartel = await dossierDcim.getDirectoryHandle("PhotoCartel", {
    create: true,
  });

  const dossiersCreesOuOuverts = [];

  for (const nomDossier of DOSSIERS_INFRASTRUCTURE_PHOTOCARTEL) {
    await dossierPhotoCartel.getDirectoryHandle(nomDossier, { create: true });
    dossiersCreesOuOuverts.push(nomDossier);
  }

  return {
    dossierPhotoCartel,
    dossiersCreesOuOuverts,
  };
}

async function testerStockageAndroid() {
  setMessageTestStockageAndroid(
    "Sélecteur Android ouvert : sélectionne exactement DCIM. PhotoCartel créera ensuite DCIM/PhotoCartel et ses dossiers d'infrastructure."
  );
  ...
  try {
    ...
    etape = "création ou ouverture de DCIM/PhotoCartel et des dossiers d'infrastructure";
    const { dossierPhotoCartel, dossiersCreesOuOuverts } =
    ...
    const nomFichierTest = `test_stockage_photocartel_v28_${horodatage}.txt`;
    ...
    const contenuTest =
      "Test stockage PhotoCartel v28 OK\n" +
      "Chemin cible demandé : DCIM / PhotoCartel / Logs\n" +
      "Dossiers infrastructure : " +
      dossiersCreesOuOuverts.join(", ") +
      "\n" +
    ...
    if (!contenuRelu.includes("Test stockage PhotoCartel v28 OK")) {
      throw new Error("Le fichier test a été créé mais sa relecture est incohérente.");
    }

    setMessageTestStockageAndroid(
      "Test réussi : DCIM/PhotoCartel est créé ou accessible avec ses dossiers d'infrastructure. " +
...
const validerNouvelleVisite = async () => {
  ...
  try {
    ...
    // v27.3.1 : IMPORTANT Android.
    // showDirectoryPicker() doit être appelé directement depuis le clic utilisateur.
    // On crée donc les dossiers Android AVANT les appels serveur async, sinon Chrome perd
    // le geste utilisateur et refuse le sélecteur de dossier.
    try {
      resultatArborescenceAndroid = await creerDossiersNouvelleVisiteAndroid({
        voyageNom: voyage,
        villeNom: ville,
        visiteNom: lieu,
        typeVisiteNom: typeNouvelleVisite,
      });
    } catch (errorAndroid) {
      console.error("Erreur création arborescence Android", errorAndroid);
      alert(
        "Création de la visite annulée : les dossiers Android n'ont pas été créés.\n\n" +
    ...
    if (
      estAndroid() &&
      resultatArborescenceAndroid &&
      !resultatArborescenceAndroid.success &&
      !resultatArborescenceAndroid.ignore
    ) {
      alert(
        resultatArborescenceAndroid.raison ||
    ...
    if (resultatArborescenceAndroid?.success) {
      setMessageArborescenceAndroid(
        "Dossiers Android créés : " + resultatArborescenceAndroid.cheminLisible
      );
    } else {
