...
const VERSION = {
  ...
  descriptif: "Galerie : image de la fiche affichée chargée explicitement",
  date: "2026-08-03",
  build: 5,
};
...
async function chargerImageGalerieAndroid(nomPhoto) {
  ...
  try {
    ...
    if (!dossierPhotosAnalysees) {
      ...
      // PhotoCartel n'a rien à faire dans l'affichage d'une image, et c'est ce
      // qui rendait le chargement plus long qu'un balayage.
...
async function resynchroniserGalerieAnalysesAndroid(listeAffichee) {
  ...
  try {
    // v69.1 — la resynchronisation n'a pas à recréer l'infrastructure : elle suit
    // un chargement qui l'a déjà fait, et son coût retardait l'affichage de l'image.
    const dossierPhotosAnalysees = await obtenirDossierPhotosAnalyseesAndroid({
      demanderPermission: false,
      verifierInfrastructure: false,
    });
        ...
        >
          <main style={{ ...styles.analyseTelephone, paddingBottom: "148px" }}>
            ...
            {galerieAnalyses.length > 0 && (() => {
              ...
              // v69.1 — Android : l'URL n'est utilisée que si elle appartient à
              // la fiche affichée. Tant que l'image n'est pas résolue, aucune
              // image n'est montrée — jamais celle d'une autre fiche.
              // PC : URL HTTP portée par la fiche, inchangée.
