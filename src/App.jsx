...
const VERSION = {
  numero: "v28.2.8",
  descriptif: "dossiers métier - autorisation Android unique",
  date: "2026-07-07",
  build: 8,
};
...
// PhotoCartel v28.2.8 — dossiers métier : création réelle Android de la visite.
...
async function sauvegarderHandleDcimAndroid(handle) {
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      transaction.objectStore("handles").put(handle, "dcim");
      transaction.oncomplete = () => resolve();
      transaction.onerror = () => reject(transaction.error);
    });
  } catch (error) {
    console.warn("Handle DCIM non sauvegardé en IndexedDB :", error);
  }
}
...
async function lireHandleDcimAndroid() {
  try {
    ...
    return await new Promise((resolve, reject) => {
      ...
      const requete = transaction.objectStore("handles").get("dcim");
      requete.onsuccess = () => resolve(requete.result || null);
      requete.onerror = () => reject(requete.error);
    });
  } catch (error) {
    console.warn("Handle DCIM non relu depuis IndexedDB :", error);
    return null;
  }
}
...
async function handleDcimEncoreAutorise(handle) {
  ...
  try {
    ...
    console.warn("Permission DCIM non vérifiable :", error);
    return false;
  }
}

async function obtenirDossierDcimAndroid({ ouvrirSelecteurSiNecessaire = true } = {}) {
  // v28.2.8 : point unique d'autorisation Android.
  // On ne déclenche plus requestPermissionSupprimeV2828().
  // Le seul geste utilisateur est showDirectoryPicker() quand aucun handle DCIM autorisé n'existe.
  ...
  const handleSauvegarde = await lireHandleDcimAndroid();
  if (handleSauvegarde && (await handleDcimEncoreAutorise(handleSauvegarde))) {
    photoCartelHandleDcimSession = handleSauvegarde;
    return handleSauvegarde;
  }
...
async function creerDossiersNouvelleVisiteAndroid({ voyageNom, villeNom, visiteNom, typeVisiteNom }) {
  // v28.2.8 : création réelle sur Android dans DCIM/PhotoCartel/Voyages/<Voyage>/<Ville>/<Visite>.
  ...
  setMessageArborescenceAndroid(
    "Création Android : PhotoCartel réutilise l'autorisation DCIM si elle existe. Sinon, sélectionne DCIM une seule fois."
  );
...
async function testerStockageAndroid() {
  setMessageTestStockageAndroid(
    "Test stockage Android : sélectionne DCIM si Android le demande. PhotoCartel vérifiera ou créera ensuite DCIM/PhotoCartel et ses dossiers d'infrastructure."
  );
  ...
  let etape = "obtention du dossier DCIM";

  try {
    const dossierDcim = await obtenirDossierDcimAndroid();

    etape = "initialisation PhotoCartel";
    const resultat = await creerArborescenceInfrastructurePhotoCartel(dossierDcim);
    ...
    setMessageArborescenceAndroid("Stockage Android initialisé : DCIM / PhotoCartel");
  } catch (error) {
    console.error(error);
    setMessageTestStockageAndroid(
