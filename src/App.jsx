...
const VERSION = {
  numero: "v30.1",
  descriptif: "rangement automatique des photos de visite",
  date: "2026-07-09",
  build: 1,
};
...
// PhotoCartel v30.1 — rangement automatique des photos de visite.
...
const afficherMessageDiscretArborescence = (message, duree = 3000) => {
  ...
  async function handleRangerPhotosVisites() {
    ...
    try {
      ...
      setDerniereActionVisite(`✅ ${total} photo(s) rangée(s) dans ${visitesTraitees} visite(s).`);
      setMessageActualisation(`✅ ${total} photo(s) rangée(s) dans ${visitesTraitees} visite(s).`);
      afficherMessageDiscretArborescence(`✅ ${total} photo(s) rangée(s).`);
    } catch (error) {
      console.error(error);
      setMessageActualisation("Erreur rangement photos : " + (error?.message || String(error)));
      alert("Erreur rangement photos : " + (error?.message || String(error)));
    } finally {
...
async function obtenirDossierRacinePhotoCartelAndroid({ ouvrirSelecteurSiNecessaire = true } = {}) {
  ...
  if (photoCartelHandleDcimSession && (await handleDcimEncoreAutorise(photoCartelHandleDcimSession))) {
    ...
    photoCartelHandleRacineAndroidSession = dossierPhotoCartel;
...
function ouvrirSelectionActualisationPhotos() {
  // v30.1 : le bouton Ranger ne demande plus de sélectionner des fichiers.
...
async function handlePhotosPrises(event) {
  ...
  async function finDeVisite() {
    ...
    try {
      ...
      // v30.1 : clôture de la fenêtre temporelle de la visite en cours.
...
const validerNouvelleVisite = async () => {
  ...
  try {
    ...
    // v30.1 : fermeture éventuelle du dossier tampon actif, puis ouverture de la vraie visite
