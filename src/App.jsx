// PhotoCartel v49 — ajout du fichier Résumé de visite.json écrit à la racine de chaque visite (identité physique, portable).
...
const VERSION = {
  ...
  descriptif: "Ajout du fichier Résumé de visite.json (identité physique de chaque visite)",
  date: "2026-07-29",
  build: 1,
};
...
const afficherMessageDiscretArborescence = (message, duree = 5000) => {
  ...
  async function modifierIdentiteVisiteAndroid(payload) {
    ...
    if (memeParent && memeNom) {
      ...
      try {
        await ecrireResumeVisiteAndroid(ancienDossier, {
          nom: payload.nouveauNom,
          voyage: payload.voyage,
          ville: payload.nouvelleVille,
          type: payload.nouveauType,
          debutMs: payload.debutMs,
          finMs: payload.finMs,
          nombrePhotos: payload.nombrePhotos,
        });
      } catch (error) {
        console.warn("Résumé de visite.json non mis à jour :", error);
      }
      return {
        success: true,
    ...
    try {
      ...
      try {
        await ecrireResumeVisiteAndroid(destination, {
          nom: payload.nouveauNom,
          voyage: payload.voyage,
          ville: payload.nouvelleVille,
          type: payload.nouveauType,
  ...
  async function enregistrerModificationIdentiteVisite() {
    ...
    const payload = {
      ...
      // v49 : identité complète transmise pour tenir à jour Résumé de visite.json.
      debutMs: ancienne.debutMs || null,
      finMs: ancienne.finMs || null,
      nombrePhotos: Number(
        ancienne.nombrePhotos ?? ancienne.photosRangees ?? ancienne.nombrePhotosDeclare ?? 0
      ),
    };
  ...
  async function rangerPhotosVisitesAndroid(visitesAtraiter) {
    ...
    // v49 : le rangement est le moment où le nombre de photos et la date de fin
    // deviennent définitifs. Résumé de visite.json est réécrit avec ces valeurs
    // réelles, uniquement pour les visites effectivement traitées ici.
    for (const visite of visitesResultats) {
      if (!visite.photosCandidates) continue;
      try {
        const dossierVisiteDestination = await obtenirDossierVisiteAndroid(dossierPhotoCartel, visite);
        await ecrireResumeVisiteAndroid(dossierVisiteDestination, {
          nom: visite.nom,
        ...
        console.warn("Résumé de visite.json non mis à jour après rangement :", visite.nom, error);
      }
    }
...
async function creerDossiersNouvelleVisiteAndroid({ voyageNom, villeNom, visiteNom, typeVisiteNom }) {
  ...
  // v49 : la visite écrit son identité dès sa création, y compris pour une visite
  // rapide (ville affichée normalisée, puisque "Visites rapides" n'est qu'un nom
  // de dossier de stockage, jamais une ville réelle à afficher à l'utilisateur).
  const villeAffichee = nomVille === "Visites rapides" ? "Ville non renseignée" : nomVille;
  try {
    await ecrireResumeVisiteAndroid(dossierVisite, {
      nom: nomVisite,
      voyage: nomVoyage,
      ville: villeAffichee,
      type: typeVisiteNom || "",
    ...
    console.warn("Résumé de visite.json non écrit à la création :", error);
  }
...
// v49 — Résumé de visite.json : identité de la visite écrite physiquement à la
// racine de son dossier, à côté de Oeuvres/Cartels/etc. Contenu minimal et stable :
// nom, voyage, ville, type, dates de début/fin, nombre de photos. Ce fichier ne
// remplace pas le cache local (toujours utilisé pour l'affichage rapide), il lui
// donne une source de vérité physique, portable d'un téléphone à l'autre.
...
async function ecrireResumeVisiteAndroid(dossierVisite, donnees) {
  await ecrireBlobDansDossierAndroid(
    dossierVisite,
    NOM_FICHIER_RESUME_VISITE,
    new Blob([JSON.stringify(construireResumeVisite(donnees), null, 2)], {
