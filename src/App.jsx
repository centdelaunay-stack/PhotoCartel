// PhotoCartel v40.10 — collecte métier unifiée : prise de photo et rangement utilisent toujours la racine officielle PhotoCartel.
...
function App() {
  ...
  async function verifierServeurModificationVisite() {
    ...
    if (String(informations.version || "") !== "v40.10") {
      throw new Error(
        `Le serveur actif annonce ${informations.version || "une version inconnue"}, pas v40.10. ` +
...
const afficherMessageDiscretArborescence = (message, duree = 5000) => {
  ...
  function dossierRacineMetierEnvoyeAuServeur() {
    // v40.10 : les opérations métier de visite (collecte et rangement)
  ...
  async function rangerPhotosVisitesAndroid(visitesAtraiter) {
    // v40.10 : aucun fichier source n'est supprimé avant une vérification complète
    ...
    for (const item of fichiersACopier) {
      ...
      try {
        ...
        try {
          await dossierCollecte.getFileHandle(nomFichier, { create: false });
          throw new Error("Le fichier source est encore présent après suppression.");
        } catch (verificationSuppressionError) {
          if (verificationSuppressionError?.name !== "NotFoundError") {
            throw verificationSuppressionError;
          }
        }
        ...
        try {
          let sourceExiste = true;
          try {
            await dossierCollecte.getFileHandle(nomFichier, { create: false });
          } catch (sourceError) {
            sourceExiste = sourceError?.name !== "NotFoundError";
          }

          if (sourceExiste && dossierDestination && nomDestination) {
