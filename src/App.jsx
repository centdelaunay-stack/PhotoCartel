// PhotoCartel v40.3 — identité de stockage persistante et résolution robuste du dossier de visite.
...
function App() {
  ...
  async function verifierServeurModificationVisite() {
    ...
    if (String(informations.version || "") !== "v40.3") {
      throw new Error(
        `Le serveur actif annonce ${informations.version || "une version inconnue"}, pas v40.3. ` +
...
const afficherMessageDiscretArborescence = (message, duree = 5000) => {
  ...
  async function trouverDossierVisiteAndroidDansVoyage(
    ...
    const parentsAParcourir = [];
    const dejaAjoutes = new Set();

    const ajouterParent = async (nomParent) => {
      const nom = nettoyerNomDossierLocal(nomParent);
      if (!nom || dejaAjoutes.has(nom)) return;
      try {
        const handle = await voyageHandle.getDirectoryHandle(nom, { create: false });
        parentsAParcourir.push({ nom, handle });
        dejaAjoutes.add(nom);
      } catch (error) {
    ...
    await ajouterParent(villeStockagePreferee);
    for await (const [nom, handle] of voyageHandle.entries()) {
      if (handle.kind === "directory" && !dejaAjoutes.has(nom)) {
        parentsAParcourir.push({ nom, handle });
        dejaAjoutes.add(nom);
      }
    }
    ...
    for (const parent of parentsAParcourir) {
      try {
        const dossier = await parent.handle.getDirectoryHandle(nomNettoye, {
          create: false,
        });
  ...
  async function modifierIdentiteVisiteAndroid(payload) {
    ...
    const nouveauParent = await voyageHandle.getDirectoryHandle(
    ...
    if (memeParent && memeNom) {
      for (const categorie of categoriesPourTypeVisiteLocale(payload.nouveauType)) {
        await ancienDossier.getDirectoryHandle(categorie, { create: true });
      }
      return {
        success: true,
    ...
    try {
      await nouveauParent.getDirectoryHandle(nouveauNomNettoye, { create: false });
      throw new Error("Une visite porte déjà ce nom dans cette ville.");
    } catch (error) {
      if (error?.message === "Une visite porte déjà ce nom dans cette ville.") throw error;
      if (error?.name !== "NotFoundError") throw error;
    }

    const nouveauDossier = await nouveauParent.getDirectoryHandle(nouveauNomNettoye, { create: true });
    ...
    try {
      ...
      const visiteMaj =
        historiqueMaj.find((v) =>
          ancienne.id ? v.id === ancienne.id : (!v.finMs && v.nom === nom)
        ) || {
          ...ancienne,
          nom,
          ville,
...
const validerNouvelleVisite = async () => {
  ...
  const baseStyles = {
    ...
    resumeVisiteStatutZone: {
      marginTop: "3px",
      paddingTop: "1px",
      borderTop: `2px solid ${themePhotoCartel.bordureOr}`,
    },
        ...
        <div style={{ ...styles.modalOverlay, zIndex: 10030 }}>
          <div style={styles.modal} role="dialog" aria-modal="true" aria-labelledby="titre-modifier-identite-visite">
            ...
            <div style={{ position: "relative", marginBottom: "10px" }}>
              ...
              <button
                type="button"
                onClick={() => setListeTypesModificationOuverte((ouverte) => !ouverte)}
                    ...
                    return (
                      <button
                        ...
                        style={{
                          ...
                          background: estTypeConfirme ? "rgba(181, 138, 58, 0.11)" : "transparent",
                          display: "grid",
