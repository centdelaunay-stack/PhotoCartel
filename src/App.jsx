...
const VERSION = {
  ...
  descriptif: "Galerie des photos analysées : index durable, affichage immédiat",
  date: "2026-08-03",
  build: 5,
};
...
// v69 — URL de l'image de la SEULE fiche affichée, créée à la demande côté
// Android puis libérée quand on quitte la fiche (avant v69, les 80 URLs étaient
// créées à l'ouverture de la galerie et gardées jusqu'à la suppression).
const [galerieImageUrlCourante, setGalerieImageUrlCourante] = useState("");
...
const afficherMessageDiscretArborescence = (message, duree = 5000) => {
  ...
  // v69 — CHARGEMENT PARESSEUX DE L'IMAGE DE LA GALERIE (Android).
  // Avant v69, l'ouverture de la galerie créait une URL objet pour les 80 photos
  // alors que l'écran n'en affiche qu'une : 80 ouvertures de fichier inutiles, et
  // 80 URLs jamais libérées. Désormais une seule image est ouverte, celle de la
  // fiche affichée, et son URL est révoquée dès qu'on quitte la fiche.
  // Sur PC l'image est servie en HTTP et le navigateur la charge déjà à la
  // demande : cet effet ne concerne donc que le chemin Android.
    ...
    (async () => {
      try {
        const dossierPhotosAnalysees = await obtenirDossierPhotosAnalyseesAndroid({
          demanderPermission: false,
        });
        if (annule) return;

        const handlePhoto = await dossierPhotosAnalysees.getFileHandle(
          nomPhotoGalerieAffichee
        );
        const fichierPhoto = await handlePhoto.getFile();
        if (annule) return;

        urlCreee = URL.createObjectURL(fichierPhoto);
        setGalerieImageUrlCourante(urlCreee);
      } catch (error) {
        // Photo absente ou accès refusé : la fiche s'affiche sans son image,
        // exactement comme le faisait la lecture par fiche avant v69.
        if (!annule) setGalerieImageUrlCourante("");
      }
    })();
  ...
  }, [modeGalerieAnalyses, nomPhotoGalerieAffichee]);
...
async function obtenirDossierPhotosAnalyseesAndroid({ demanderPermission = true } = {}) {
  const resultatRacineAndroid = await obtenirDossierRacinePhotoCartelAndroid({
    ouvrirSelecteurSiNecessaire: false,
  ...
  return await dossierPhotoCartel.getDirectoryHandle("Photos analysées", {
    create: true,
  });
}
...
async function chargerGalerieDepuisIndexAndroid() {
  const fichesIndex = await lireIndexGalerieAndroid(
    await obtenirDossierPhotosAnalyseesAndroid()
  );

  if (!fichesIndex || fichesIndex.length === 0) return null;

  // imageExiste n'est pas stocké dans l'index : il est supposé vrai le temps que
  // la resynchronisation liste le dossier et le recalcule. Une valeur fausse ici
  // n'empêche pas l'image de s'afficher (le chargement paresseux tente quand même
  // l'ouverture) ; elle ne change que le libellé « Photo introuvable ».
  return trierFichesGalerie(
    fichesIndex.map((fiche) => ({
      ...normaliserFichePourIndexGalerie(fiche),
      dossierDestination: DOSSIER_DESTINATION_GALERIE_ANDROID,
      imageExiste: true,
      imageUrl: "",
  ...
  try {
    ...
    if (
      ...
      setGalerieIndex((ancienIndex) => {
        ...
        if (nouvelIndex >= 0) return nouvelIndex;
        if (!fiches.length) return 0;
        return Math.min(ancienIndex, fiches.length - 1);
      });
      setMessageGalerieAnalyses(
...
async function supprimerFicheGalerie() {
  ...
  try {
    ...
    setGalerieIndex(Math.min(galerieIndex, nouvelleGalerie.length - 1));
    setMessageGalerieAnalyses("");
  } catch (error) {
    console.error(error);
        ...
        >
          <main style={{ ...styles.analyseTelephone, paddingBottom: "148px" }}>
            ...
            {galerieAnalyses.length > 0 && (() => {
              ...
              // v69 — Android : URL objet de la seule fiche affichée, créée à la
              // demande. PC : URL HTTP portée par la fiche, inchangée.
              const photoGalerieUrl = estAndroid()
                ? galerieImageUrlCourante
        ...
        <div style={styles.modalOverlay}>
          <div style={styles.modal}>
            ...
            <BoutonMenuPopup
              ...
              onClick={() => afficherMessageSousBouton("recherches-criteres")}
            />
            <MessageSousBouton cible="recherches-criteres" />
            <BoutonMenuPopup
              icone="💬"
