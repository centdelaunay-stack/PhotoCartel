...
const VERSION = {
  numero: "v34.4",
  descriptif: "enregistrement des modifications",
  date: "2026-07-13",
  build: 4,
};
...
// PhotoCartel v34.4 — enregistrement fiable des analyses modifiées et autorisations déclenchées au bon moment.
...
async function obtenirDossierRacinePhotoCartelAndroid({
  ...
  if (
    ...
    (await handleAndroidEncoreAutorise(photoCartelHandleRacineAndroidSession, { demanderPermissionSiNecessaire }))
  ) {
    return {
      type: "photocartel",
  ...
  if (photoCartelHandleDcimSession && (await handleDcimEncoreAutorise(photoCartelHandleDcimSession, { demanderPermissionSiNecessaire }))) {
...
async function handleAnalyserUnePhoto() {
  // v34.4 : l'ouverture de la fonctionnalité ne demande plus d'autorisation d'écriture.
  // L'accès au dossier est vérifié uniquement quand l'utilisateur choisit réellement
  // de prendre ou sélectionner une photo, donc juste avant la première écriture.
  fermerAnalysePhoto();
...
async function preparerAccesEcritureAnalyseAndroid() {
  if (!estAndroid()) return true;

  try {
    const resultat = await obtenirDossierRacinePhotoCartelAndroid({
      ouvrirSelecteurSiNecessaire: true,
      demanderPermissionSiNecessaire: true,
    });
    return Boolean(resultat?.dossierPhotoCartel);
  } catch (error) {
    console.error("Accès au dossier PhotoCartel refusé :", error);
    window.alert(
      "PhotoCartel doit accéder au dossier PhotoCartel pour enregistrer la photo. " +
        error.message
    );
    return false;
  }
...
async function choisirPrendrePhotoPourAnalyse() {
  // v34.4 : la permission n'est demandée qu'au moment où une photo va réellement
  // être créée dans « Photos à analyser », jamais au simple affichage du menu.
  if (!(await preparerAccesEcritureAnalyseAndroid())) return;
...
async function handlePhotoAnalyseSelection(event) {
  ...
  try {
    ...
    console.error(error);
    setMessageAnalysePhoto("Erreur d’enregistrement dans Photos à analyser : " + error.message);
  } finally {
    event.target.value = "";
  }
}
...
async function lancerAnalysePhotoIA() {
  ...
  // v34.3 : ce clic est un geste utilisateur explicite. Il permet de
  // réactiver l'autorisation du dossier avant tout traitement asynchrone.
  if (estAndroid()) {
    try {
      ...
      setMessageAnalysePhoto("Accès au dossier PhotoCartel impossible : " + error.message);
      return;
    }
  }
  ...
  try {
    ...
    if (estAndroid()) {
      ...
      sauvegardeForm.append("analyse", JSON.stringify(data.result));
...
async function enregistrerAnalysePhoto() {
  ...
  try {
    ...
    let data;
    if (estAndroid()) {
      // Ce clic est un geste utilisateur : on réactive ici la permission si Android
      // l'a perdue, puis on réutilise exactement le même handle pendant toute l'écriture.
...
const validerNouvelleVisite = async () => {
  ...
  function BarreActionsFicheResultat() {
    const actions = [
      {
        icone: "✏️",
        texte: "Modifier l'analyse",
        // Un nouveau clic en mode édition est volontairement sans effet.
        action: () => {
          if (!analysePhotoEdition) setAnalysePhotoEdition(true);
        },
        disabled: !analysePhotoResultat || analysePhotoEnCours,
      },
      {
        icone: "💾",
          ...
          <main style={{ ...styles.analyseTelephone, paddingBottom: "148px" }}>
            ...
            {!analysePhotoResultat && !analysePhotoEnCours && (
              <div style={styles.analyseCarte}>
                ...
                <button type="button" onClick={lancerAnalysePhotoIA} style={styles.boutonLancerAnalysePrincipal} disabled={!analysePhotoFile}>
                  🤖 Lancer l'analyse IA
                </button>
