...
const VERSION = {
  numero: "v28.2.4",
  descriptif: "dossiers métier - création de la visite sous Ville",
  date: "2026-07-07",
  build: 4,
};
...
// PhotoCartel v28.2.4 — dossiers métier : création de la visite sous Ville.
// Socle v28.1 conservé : infrastructure idempotente, moteurs IA/OCR/classification/renommage inchangés.
...
async function creerArborescenceAndroidSurTelephone({ voyageNom, villeNom, lieuNom, typeVisiteNom }) {
  ...
  const continuer = window.confirm(
    "PhotoCartel va créer le vrai dossier sur le téléphone.\n\n" +
...
async function creerDossiersNouvelleVisiteAndroid({ voyageNom, villeNom, visiteNom, typeVisiteNom }) {
  // v27.3 : intégration définitive de la création Android dans le flux normal "Nouvelle visite".
  // L'utilisateur choisit uniquement le dossier racine déjà créé : DCIM / PhotoCartel.
  // PhotoCartel crée ensuite automatiquement Voyage / Ville / Visite / sous-dossiers.
  if (!estAndroid()) {
    return {
      success: false,
      ignore: true,
  ...
  if (typeof window.showDirectoryPicker !== "function") {
    throw new Error(
      "PhotoCartel ne voit pas l'API de sélection de dossier.\n\n" +
        diagnostic +
        "\n\nOuvre PhotoCartel depuis l'icône PWA / Chrome Android à jour, puis réessaie."
    );
  }

  const continuer = window.confirm(
    "PhotoCartel va préparer les vrais dossiers de la visite sur le téléphone.\n\n" +
      "Dans l'écran suivant, choisis uniquement :\n\n" +
      "DCIM / PhotoCartel\n\n" +
      "Ne choisis pas DCIM.\n" +
      "Ne choisis pas Camera.\n" +
      "Choisis directement le dossier PhotoCartel.\n\n" +
      "PhotoCartel créera ensuite automatiquement :\n" +
      voyageNom + " / " + villeNom + " / " + visiteNom
  );
  ...
  if (!continuer) {
    return {
      success: false,
      ignore: false,
      raison: "Création des dossiers Android annulée par l'utilisateur.",
    };
  }
  ...
  const dossierVoyages = await racinePhotoCartel.getDirectoryHandle(
  ...
  return {
    success: true,
    cheminLisible:
      "DCIM / PhotoCartel / Voyages / " + nomVoyage + " / " + nomVille + " / " + nomVisite,
...
async function creerArborescenceInfrastructurePhotoCartel(dossierDcim) {
  // v28.1 : point unique de création / réparation de l'arborescence système.
  // L'utilisateur sélectionne DCIM ; PhotoCartel ouvre ou crée DCIM/PhotoCartel,
  // puis vérifie un par un les dossiers d'infrastructure.
  // Cas essentiel : si DCIM/PhotoCartel existe déjà avec des dossiers métier,
  // on les conserve et on ajoute uniquement les dossiers d'infrastructure manquants.
...
async function creerDossierVoyageMetierAndroid({ voyageNom }) {
  // v28.2 : création métier Android du voyage dans DCIM/PhotoCartel/Voyages/<Voyage>.
  // Cette fonction réutilise le moteur d'infrastructure v28.1 : elle ne supprime rien,
  // ne déplace rien et crée uniquement les dossiers manquants.
  if (!estAndroid()) {
    return {
      success: false,
      ignore: true,
  ...
  const dossierDcim = await window.showDirectoryPicker({
    id: "photocartel-dcim-v28-2-voyage",
    mode: "readwrite",
    startIn: "pictures",
  });

  if (dossierDcim.name !== "DCIM") {
    throw new Error(
      "Mauvais dossier sélectionné. Dossier reçu : " +
        dossierDcim.name +
        ". Il faut sélectionner exactement DCIM."
    );
  }

  if (dossierDcim.queryPermission) {
    const permissionLectureEcriture = await dossierDcim.queryPermission({
      mode: "readwrite",
    });

    if (permissionLectureEcriture !== "granted" && dossierDcim.requestPermission) {
      const permissionDemandee = await dossierDcim.requestPermission({
        mode: "readwrite",
      });

      if (permissionDemandee !== "granted") {
        throw new Error(
          "Autorisation refusée par Android/Chrome. Permission obtenue : " +
            permissionDemandee
        );
      }
    }
  ...
  const nomVoyage = nettoyerNomDossierLocal(voyageNom);
  await dossierVoyages.getDirectoryHandle(nomVoyage, { create: true });

  return {
    success: true,
    cheminLisible: "DCIM / PhotoCartel / Voyages / " + nomVoyage,
  };
}

async function testerStockageAndroid() {
  setMessageTestStockageAndroid(
    "Sélecteur Android ouvert : sélectionne exactement DCIM. PhotoCartel ouvrira ou créera DCIM/PhotoCartel, puis ajoutera uniquement les dossiers d'infrastructure manquants."
  );
  ...
  let etape = "ouverture du sélecteur Android";

  try {
    // IMPORTANT : showDirectoryPicker doit être appelé directement depuis le clic utilisateur.
    // Aucun alert(), confirm() ni await ne doit être exécuté avant cet appel, sinon Chrome bloque le sélecteur.
    const dossierDcim = await window.showDirectoryPicker({
      id: "photocartel-dcim-v28-1",
      mode: "readwrite",
      startIn: "pictures",
    });

    etape = "contrôle du dossier sélectionné";
    if (dossierDcim.name !== "DCIM") {
      throw new Error(
        "Mauvais dossier sélectionné. Dossier reçu : " +
          dossierDcim.name +
          ". Il faut sélectionner exactement DCIM, pas PhotoCartel, pas Collecte Photo en cours, pas un dossier de visite."
      );
    }

    etape = "contrôle de l'autorisation readwrite";
    if (dossierDcim.queryPermission) {
      const permissionLectureEcriture = await dossierDcim.queryPermission({
        mode: "readwrite",
      });
      ...
      if (permissionLectureEcriture !== "granted" && dossierDcim.requestPermission) {
        etape = "demande d'autorisation readwrite";
        const permissionDemandee = await dossierDcim.requestPermission({
          mode: "readwrite",
        });
    ...
    etape = "ouverture ou création de DCIM/PhotoCartel et vérification idempotente des dossiers d'infrastructure";
    const {
      dossierPhotoCartel,
      statutRacine,
      dossiersCrees,
      dossiersExistants,
      dossiersVerifies,
    } = await creerArborescenceInfrastructurePhotoCartel(dossierDcim);

    etape = "ouverture du dossier Logs";
    const dossierLogs = await dossierPhotoCartel.getDirectoryHandle("Logs", {
      create: true,
    });

    etape = "création du fichier test";
    ...
    const nomFichierTest = `test_stockage_photocartel_v28_1_${horodatage}.txt`;
    const fichierTest = await dossierLogs.getFileHandle(nomFichierTest, {
      create: true,
    });

    etape = "écriture du fichier test";
    const contenuTest =
      "Test stockage PhotoCartel v28.1 OK\n" +
      "Chemin cible demandé : DCIM / PhotoCartel / Logs\n" +
      "Racine PhotoCartel : " +
      statutRacine +
      "\n" +
      "Dossiers créés : " +
      (dossiersCrees.length ? dossiersCrees.join(", ") : "aucun") +
      "\n" +
      "Dossiers déjà présents : " +
      (dossiersExistants.length ? dossiersExistants.join(", ") : "aucun") +
      "\n" +
      "Dossiers vérifiés : " +
      dossiersVerifies.join(", ") +
      "\n" +
      `Dossier sélectionné : ${dossierDcim.name}\n` +
      `Fichier : ${nomFichierTest}\n` +
    ...
    etape = "relecture du fichier test";
    const fichierRelu = await fichierTest.getFile();
    const contenuRelu = await fichierRelu.text();

    if (!contenuRelu.includes("Test stockage PhotoCartel v28.1 OK")) {
      throw new Error("Le fichier test a été créé mais sa relecture est incohérente.");
    }

    setMessageTestStockageAndroid(
      "Test réussi : DCIM/PhotoCartel est accessible. Initialisation idempotente terminée. " +
        "Créés : " +
        (dossiersCrees.length ? dossiersCrees.join(", ") : "aucun") +
        ". Déjà présents : " +
        (dossiersExistants.length ? dossiersExistants.join(", ") : "aucun") +
        ". Fichier écrit et relu dans Logs : " +
        nomFichierTest
    );
  } catch (error) {
...
function handlePhotosPrises(event) {
  ...
  async function finDeVisite() {
    ...
    if (estTamponActif && cheminCollecteActif) {
      ...
      cheminParentTampon = cheminVilleMetier(voyage, villeVisite);
    } else {
      cheminParentTampon = cheminVoyageMetier(voyage);
    }
...
const validerNouveauVoyage = async () => {
  ...
  try {
    let resultatVoyageAndroid = null;

    try {
      resultatVoyageAndroid = await creerDossierVoyageMetierAndroid({
        voyageNom: nomVoyage,
      });
    } catch (errorAndroid) {
      console.error("Erreur création voyage Android", errorAndroid);
      alert(
        "Création du voyage annulée : le dossier Android n'a pas été créé.\n\n" +
    ...
    if (
      estAndroid() &&
      resultatVoyageAndroid &&
      !resultatVoyageAndroid.success &&
      !resultatVoyageAndroid.ignore
    ) {
      alert(
        resultatVoyageAndroid.raison ||
          "Création du voyage annulée : dossier Android non sélectionné."
      );
      return;
    }
...
setTypeNouvelleVisite("Musée");
  ...
  localStorage.setItem("photoCartelPhotosCollectees", "0");
    ...
    if (resultatVoyageAndroid?.success) {
      setMessageArborescenceAndroid(
        "Dossier voyage Android créé : " + resultatVoyageAndroid.cheminLisible
      );
    } else {
      setMessageArborescenceAndroid("Dossier voyage créé : " + cheminVoyageServeur);
    }
...
const validerNouvelleVisite = async () => {
  // v28.2.4 : création complète de la visite métier.
  // Modèle validé : PhotoCartel / Voyages / Voyage / Ville / Nom de la visite.
  // Le champ "Nom de la visite" remplace définitivement l'ancien libellé "Lieu".
  ...
  try {
    let cheminVisiteServeur = cheminVisite;
    let data = null;
    ...
    try {
      ...
      cheminVisiteServeur = data.chemin || cheminVisite;
      ...
      cheminVisiteServeur = dataFallback.chemin || cheminVisite;
    }
    ...
    setDossierTampon("");
    setCheminTamponActif("");
    setDerniereActionVisite("Visite créée le " + formaterDate(new Date()));
    setPhotosCollectees(0);
    localStorage.setItem("photoCartelPhotosCollectees", "0");
    setMessageActualisation("");
    setDerniereActualisation(null);
    ...
    setMessageArborescenceAndroid("Dossier visite créé : " + cheminVisiteServeur);
