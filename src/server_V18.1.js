...
const DOSSIER_RACINE_PHOTOCARTEL = __dirname;
const DOSSIER_EXPORT_PHOTOCARTEL = path.join(
  DOSSIER_RACINE_PHOTOCARTEL,
  "Export"
);

fs.mkdirSync(DOSSIER_EXPORT_PHOTOCARTEL, { recursive: true });
console.log("Dossier Export PhotoCartel =", DOSSIER_EXPORT_PHOTOCARTEL);
...
- Si l'image ne permet pas d'être sûr, utilise les champs hypothèses et confiance.
...
Le champ "pays" doit être renseigné lorsqu'une origine, une localisation ou une culture est probable.
Le champ "ville" doit être renseigné lorsqu'une ville est probable.
...
`,
      },
      {
        ...
        content: [
          {
            type: "text",
            text: "Analyse cette photo comme une fiche patrimoniale PhotoCartel. Identifie ce que tu peux reconnaître, puis retourne uniquement le JSON demandé.",
          },
          {
            type: "image_url",
            image_url: {
...
app.get("/export-analyses-csv", async (req, res) => {
  try {
    ...
    const cheminExport = path.join(DOSSIER_EXPORT_PHOTOCARTEL, nomFichier);
    ...
    res.json({
      ...
      fichier: nomFichier,
      cheminExport,
      dossierExport: DOSSIER_EXPORT_PHOTOCARTEL,
